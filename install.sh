...
if [ -n "$binary_path" ]; then
    ...
    if [ -z "$requested_version" ]; then
        url="https://github.com/anomalyco/openhei/releases/latest/download/$filename"
        specific_version=$(curl -s https://api.github.com/repos/anomalyco/openhei/releases/latest | sed -n 's/.*"tag_name": *"v\([^"]*\)".*/\1/p')
        ...
        url="https://github.com/anomalyco/openhei/releases/download/v${requested_version}/$filename"
        specific_version=$requested_version

        # Verify the release exists before downloading
        http_status=$(curl -sI -o /dev/null -w "%{http_code}" "https://github.com/anomalyco/openhei/releases/tag/v${requested_version}")
        if [ "$http_status" = "404" ]; then
            echo -e "${RED}Error: Release v${requested_version} not found${NC}"
            echo -e "${MUTED}Available releases: https://github.com/anomalyco/openhei/releases${NC}"
            exit 1
        fi
    fi
fi
