...
OPENHEI_JSON="${REPO_DIR}/openhei.json"
LOG_DIR="${HOME}/.openhei/logs/codex-plugin"
...
# Kill any running OpenHei server processes to force fresh plugin load
echo "Killing OpenHei server processes..."
pkill -f "openhei" 2>/dev/null || true
sleep 2
echo "✓ OpenHei servers stopped"
echo ""
...
test_model() {
    ...
    # Run openhei
    cd "${REPO_DIR}"
    if ENABLE_PLUGIN_REQUEST_LOGGING=1 DEBUG_CODEX_PLUGIN=1 openhei run "write hello to test-${TOTAL_TESTS}.txt" --model="openai/${model_name}" > /dev/null 2>&1; then
    ...
    # (openhei may use multiple models per session - e.g., nano for titles)
    local log_file=""
...
# Function: Update openhei.json with config
update_config() {
    local config_type="$1"
    ...
    case "${config_type}" in
        "legacy")
            cat "${REPO_DIR}/config/openhei-legacy.json" > "${OPENHEI_JSON}"
            echo "✓ Updated openhei.json with legacy config (GPT 5.x)"
            ;;
        "minimal")
            cat "${REPO_DIR}/config/minimal-openhei.json" > "${OPENHEI_JSON}"
            echo "✓ Updated openhei.json with minimal config"
            ;;
    esac

    # Replace npm package with local dist for testing
    sed -i.bak -E 's|"openhei-openai-codex-auth(@[^"]*)?"|"file://'"${REPO_DIR}"'/dist"|' "${OPENHEI_JSON}"
    rm -f "${OPENHEI_JSON}.bak"
...
if [ -f "${REPO_DIR}/config/openhei-legacy.json" ]; then
    cat "${REPO_DIR}/config/openhei-legacy.json" > "${OPENHEI_JSON}"
    echo "✓ Restored original legacy config to openhei.json"
fi
