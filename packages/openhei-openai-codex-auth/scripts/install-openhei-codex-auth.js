...
const PLUGIN_NAME = "openhei-openai-codex-auth";
const args = new Set(process.argv.slice(2));

if (args.has("--help") || args.has("-h")) {
	console.log(`Usage: ${PLUGIN_NAME} [--modern|--legacy] [--uninstall] [--all] [--dry-run] [--no-cache-clear]\n\n` +
		"Default behavior:\n" +
		"  - Installs/updates global config at ~/.config/openhei/openhei.jsonc (falls back to .json)\n" +
		"  - Uses modern config (variants) by default\n" +
		"  - Ensures plugin is unpinned (latest)\n" +
		"  - Clears OpenHei plugin cache\n\n" +
		"Options:\n" +
		"  --modern           Force modern config (default)\n" +
		"  --legacy           Use legacy config (older OpenHei versions)\n" +
		...
		"  --no-cache-clear   Skip clearing OpenHei cache\n"
	);
	process.exit(0);
}
...
const templatePath = join(
	repoRoot,
	"config",
	useLegacy ? "openhei-legacy.json" : "openhei-modern.json"
);

const configDir = join(homedir(), ".config", "openhei");
const configPathJson = join(configDir, "openhei.json");
const configPathJsonc = join(configDir, "openhei.jsonc");
const cacheDir = join(homedir(), ".cache", "openhei");
...
const openheiAuthPath = join(homedir(), ".openhei", "auth", "openai.json");
const pluginConfigPath = join(
	homedir(),
	".openhei",
	"openai-codex-auth-config.json",
);
const pluginLogDir = join(homedir(), ".openhei", "logs", "codex-plugin");
const openheiCacheDir = join(homedir(), ".openhei", "cache");
...
async function getKnownModelIds() {
	const legacyTemplate = await readJson(
		join(repoRoot, "config", "openhei-legacy.json"),
	);
	const modernTemplate = await readJson(
		join(repoRoot, "config", "openhei-modern.json"),
	);
...
async function clearPluginArtifacts() {
	...
	const cacheFiles = [
		...
		"openhei-codex.txt",
		"openhei-codex-meta.json",
	];

	for (const file of cacheFiles) {
		const target = join(openheiCacheDir, file);
		if (dryRun) {
...
async function main() {
	...
	log("\nDone. Restart OpenHei to (re)install the plugin.");
	log("Example: openhei");
	if (useLegacy) {
		log("Note: Legacy config requires OpenHei v1.0.209 or older.");
	}
}
