...
# Tests that OpenHei correctly uses models from config
...
cd "${REPO_DIR}"
if ENABLE_PLUGIN_REQUEST_LOGGING=1 openhei run "say hello" --model="openai/gpt-5.1-codex-low" > /dev/null 2>&1; then
...
if ENABLE_PLUGIN_REQUEST_LOGGING=1 openhei run "write test" --model="openai/fake-model-xyz" > /dev/null 2>&1; then
...
if [ -f "${REPO_DIR}/openhei.json" ]; then
    plugin_path=$(jq -r '.plugin[0]' "${REPO_DIR}/openhei.json")
    model_count=$(jq '.provider.openai.models | length' "${REPO_DIR}/openhei.json")

    echo -e "  Config file:  ${REPO_DIR}/openhei.json"
    echo -e "  Plugin path:  ${plugin_path}"
    echo -e "  Models defined: ${model_count}"
    echo -e "  ${GREEN}✓ Config file found and valid${NC}"
else
    echo -e "  ${RED}✗ No openhei.json in repo directory${NC}"
    exit 1
fi
