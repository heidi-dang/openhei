/**
 * OpenHei Codex Prompt Fetcher
 *
 * Fetches and caches the codex.txt system prompt from OpenHei's GitHub repository.
...
const OPENHEI_CODEX_URL =
	"https://raw.githubusercontent.com/heidi-dang/openhei/dev/packages/openhei/src/session/prompt/codex.txt";
const CACHE_DIR = join(homedir(), ".openhei", "cache");
const CACHE_FILE = join(CACHE_DIR, "openhei-codex.txt");
const CACHE_META_FILE = join(CACHE_DIR, "openhei-codex-meta.json");
...
/**
 * Fetch OpenHei's codex.txt prompt with ETag-based caching
 * Uses HTTP conditional requests to efficiently check for updates
 *
 * Rate limit protection: Only checks GitHub if cache is older than 15 minutes
 * @returns The codex.txt content
 */
export async function getOpenHeiCodexPrompt(): Promise<string> {
	...
	try {
		const response = await fetch(OPENHEI_CODEX_URL, { headers });
		...
		throw new Error(`Failed to fetch OpenHei codex.txt: ${response.status}`);
	} catch (error) {
		...
		throw new Error(
			`Failed to fetch OpenHei codex.txt and no cache available: ${error}`
		);
	}
}

/**
 * Get first N characters of the cached OpenHei prompt for verification
