...
import { CODEX_OPENHEI_BRIDGE } from "../prompts/codex-openhei-bridge.js";
import { getOpenHeiCodexPrompt } from "../prompts/openhei-codex.js";
 ...
 *
 * NOTE: This plugin follows Codex CLI defaults instead of openhei defaults because:
 * - We're accessing the ChatGPT backend API (not OpenAI Platform API)
 * - openhei explicitly excludes gpt-5-codex from automatic reasoning configuration
 ...
 *
 * @param input - Original input array from OpenHei/AI SDK
...
/**
 * Filter out OpenHei system prompts from input
 * Used in CODEX_MODE to replace OpenHei prompts with Codex-OpenHei bridge
 * @param input - Input array
 * @returns Input array without OpenHei system prompts
 */
export async function filterOpenHeiSystemPrompts(
	input: InputItem[] | undefined,
): Promise<InputItem[] | undefined> {
	if (!Array.isArray(input)) return input;

	// Fetch cached OpenHei prompt for verification
	let cachedPrompt: string | null = null;
	try {
		cachedPrompt = await getOpenHeiCodexPrompt();
	} catch {
		// If fetch fails, fallback to text-based detection only
		// This is safe because we still have the "starts with" check
	}

	return filterOpenHeiSystemPromptsWithCachedPrompt(input, cachedPrompt);
}

/**
 * Add Codex-OpenHei bridge message to input if tools are present
 * @param input - Input array
 ...
 *
 * NOTE: Configuration follows Codex CLI patterns instead of openhei defaults:
 * - openhei sets textVerbosity="low" for gpt-5, but Codex CLI uses "medium"
 * - openhei excludes gpt-5-codex from reasoning configuration
...
export async function transformRequestBody(
	...
	// (OpenHei passes its session identifier). We no longer synthesize one here.
	...
	if (body.input && Array.isArray(body.input)) {
		...
		if (codexMode) {
			// CODEX_MODE: Remove OpenHei system prompt, add bridge prompt
			body.input = await filterOpenHeiSystemPrompts(body.input);
