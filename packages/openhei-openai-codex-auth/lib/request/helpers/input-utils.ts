...
const OPENHEI_PROMPT_SIGNATURES = [
	"you are a coding agent running in the openhei",
	"you are openhei, an agent",
	"you are openhei, an interactive cli agent",
	"you are openhei, an interactive cli tool",
	"you are openhei, the best coding agent on the planet",
].map((signature) => signature.toLowerCase());

const OPENHEI_CONTEXT_MARKERS = [
...
const extractOpenHeiContext = (contentText: string): string | null => {
	const lower = contentText.toLowerCase();
	let earliestIndex = -1;

	for (const marker of OPENHEI_CONTEXT_MARKERS) {
...
export function isOpenHeiSystemPrompt(
	item: InputItem,
	...
	return OPENHEI_PROMPT_SIGNATURES.some((signature) =>
		normalized.startsWith(signature),
	);
}

export function filterOpenHeiSystemPromptsWithCachedPrompt(
	...
	return input.flatMap((item) => {
		...
		if (!isOpenHeiSystemPrompt(item, cachedPrompt)) {
			return [item];
		}

		const contentText = getContentText(item);
		const preservedContext = extractOpenHeiContext(contentText);
		if (preservedContext) {
