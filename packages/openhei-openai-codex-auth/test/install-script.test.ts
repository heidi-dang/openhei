...
const SCRIPT_PATH = resolve(process.cwd(), 'scripts', 'install-openhei-codex-auth.js');
...
const makeHome = () => mkdtempSync(join(tmpdir(), 'openhei-install-'));

const writeConfig = (homeDir: string, file: string, content: string) => {
	const configDir = join(homeDir, '.config', 'openhei');
...
describe('Install script', () => {
	it('updates existing JSONC and preserves comments', () => {
		...
		const configPath = writeConfig(
			...
			`{
  // My existing config
  "plugin": ["some-other-plugin@1.2.3", "openhei-openai-codex-auth@4.2.0"],
  "provider": {
    "openai": {
      "timeout": 60000,
      "models": { "custom-model": { "name": "Custom" } }
		...
		expect(data.plugin).toContain('openhei-openai-codex-auth');
	...
	it('prefers JSONC when both jsonc and json exist', () => {
		...
		const jsoncPath = writeConfig(
			homeDir,
			'openhei.jsonc',
			`{ "plugin": ["openhei-openai-codex-auth@4.2.0"] }`,
		);
	...
	it('creates JSONC when no config exists', () => {
		const homeDir = makeHome();
		runInstaller(['--no-cache-clear'], homeDir);
		const configPath = join(homeDir, '.config', 'openhei', 'openhei.jsonc');
  ...
  "provider": {
	...
	it('uninstall --all removes plugin artifacts', () => {
		...
		const openheiDir = join(homeDir, '.openhei');
		mkdirSync(join(openheiDir, 'auth'), { recursive: true });
		mkdirSync(join(openheiDir, 'logs', 'codex-plugin'), { recursive: true });
		mkdirSync(join(openheiDir, 'cache'), { recursive: true });
		writeFileSync(join(openheiDir, 'auth', 'openai.json'), '{}');
		writeFileSync(join(openheiDir, 'openai-codex-auth-config.json'), '{}');
		writeFileSync(join(openheiDir, 'logs', 'codex-plugin', 'log.txt'), 'log');
		writeFileSync(join(openheiDir, 'cache', 'codex-instructions.md'), 'cache');

		runInstaller(['--uninstall', '--all', '--no-cache-clear'], homeDir);

		expect(existsSync(join(openheiDir, 'auth', 'openai.json'))).toBe(false);
		expect(existsSync(join(openheiDir, 'openai-codex-auth-config.json'))).toBe(false);
		expect(existsSync(join(openheiDir, 'logs', 'codex-plugin'))).toBe(false);
		expect(existsSync(join(openheiDir, 'cache', 'codex-instructions.md'))).toBe(false);
	});
});
