...
import {
    ...
    filterInput,
    addToolRemapMessage,
    isOpenHeiSystemPrompt,
    filterOpenHeiSystemPrompts,
    filterOpenHeiSystemPromptsWithCachedPrompt,
    addCodexBridgeMessage,
    transformRequestBody,
} from '../lib/request/request-transformer.js';
import { TOOL_REMAP_MESSAGE } from '../lib/prompts/codex.js';
import { CODEX_OPENHEI_BRIDGE } from '../lib/prompts/codex-openhei-bridge.js';
...
describe('Request Transformer Module', () => {
	...
	describe('isOpenHeiSystemPrompt', () => {
		it('should detect OpenHei system prompt with string content', async () => {
			const item: InputItem = {
				type: 'message',
				role: 'developer',
				content: 'You are a coding agent running in the openhei, a terminal-based coding assistant.',
			};
			expect(isOpenHeiSystemPrompt(item, null)).toBe(true);
		});

		it('should detect OpenHei system prompt with array content', async () => {
			const item: InputItem = {
				type: 'message',
				role: 'developer',
				content: [
					{
						type: 'input_text',
						text: 'You are a coding agent running in the openhei, a terminal-based coding assistant.',
					},
				],
			};
			expect(isOpenHeiSystemPrompt(item, null)).toBe(true);
		...
		it('should not detect non-system roles', async () => {
			...
			expect(isOpenHeiSystemPrompt(item, null)).toBe(false);
		});
		...
		it('should NOT detect content with codex signature in the middle', async () => {
			const cachedPrompt = 'You are a coding agent running in the openhei.';
			const item: InputItem = {
				type: 'message',
				role: 'developer',
				// Has codex.txt content but with environment prepended (like OpenHei does)
				content: 'Environment info here\n\nYou are a coding agent running in the openhei.',
			};
			// First 200 chars won't match because of prepended content
			expect(isOpenHeiSystemPrompt(item, cachedPrompt)).toBe(false);
		});
		...
		it('should detect with cached prompt exact match', async () => {
			...
			const item: InputItem = {
				type: 'message',
				role: 'developer',
				content: 'You are a coding agent running in the openhei',
			};
			expect(isOpenHeiSystemPrompt(item, cachedPrompt)).toBe(true);
		});

		it('should detect alternative OpenHei prompt signatures', async () => {
			const item: InputItem = {
				type: 'message',
				role: 'developer',
				content: "You are openhei, an agent - please keep going until the user's query is completely resolved.",
			};
			expect(isOpenHeiSystemPrompt(item, null)).toBe(true);
		});
	});

	describe('filterOpenHeiSystemPrompts', () => {
		it('should filter out OpenHei system prompts', async () => {
			const input: InputItem[] = [
				{
					type: 'message',
					role: 'developer',
			...
			const result = filterOpenHeiSystemPromptsWithCachedPrompt(input, null);
			expect(result).toHaveLength(1);
			expect(result![0].role).toBe('user');
		});
		...
		it('should keep non-OpenHei developer messages', async () => {
			const input: InputItem[] = [
			...
			const input: InputItem[] = [
				{
					type: 'message',
					role: 'developer',
					content: 'You are a coding agent running in the openhei', // This is codex.txt
				},
				{
					type: 'message',
					role: 'developer',
		...
		it('should strip OpenHei prompt but keep concatenated env/instructions', async () => {
			const input: InputItem[] = [
				{
					type: 'message',
					role: 'developer',
					content: [
						'You are a coding agent running in the openhei, a terminal-based coding assistant.',
			...
			expect(preserved).not.toContain('You are a coding agent running in the openhei');
		});
			...
			const input: InputItem[] = [
				{
					type: 'message',
					role: 'developer',
					content: 'You are a coding agent running in the openhei', // codex.txt alone
				},
				{
					type: 'message',
					role: 'developer',
					// environment + AGENTS.md joined (like OpenHei does)
		...
		it('should return undefined for undefined input', async () => {
			expect(await filterOpenHeiSystemPrompts(undefined)).toBeUndefined();
		});
	});
	...
	describe('addCodexBridgeMessage', () => {
		it('should prepend bridge message when tools present', async () => {
			...
			expect(result![0].role).toBe('developer');
			expect(result![0].type).toBe('message');
			expect((result![0].content as any)[0].text).toContain('Codex Running in OpenHei');
		});
		...
		describe('transformRequestBody', () => {
			...
			it('preserves existing prompt_cache_key passed by host (OpenHei)', async () => {
				const body: RequestBody = {
					model: 'gpt-5-codex',
					input: [],
					// Host-provided key (OpenHei session id)
		...
		describe('CODEX_MODE parameter', () => {
			it('should use bridge message when codexMode=true and tools present (default)', async () => {
				...
				expect(result.input).toHaveLength(2);
				expect(result.input![0].role).toBe('developer');
				expect((result.input![0].content as any)[0].text).toContain('Codex Running in OpenHei');
			});

			it('should filter OpenHei prompts when codexMode=true', async () => {
				const body: RequestBody = {
					model: 'gpt-5',
					input: [
						{
				...
				// Should have bridge message + user message (OpenHei prompt filtered out)
				expect(result.input).toHaveLength(2);
				expect(result.input![0].role).toBe('developer');
				expect((result.input![0].content as any)[0].text).toContain('Codex Running in OpenHei');
			...
			it('should not filter OpenHei prompts when codexMode=false', async () => {
				const body: RequestBody = {
					model: 'gpt-5',
					input: [
						{
				...
				// Should have tool remap + openhei prompt + user message
				expect(result.input).toHaveLength(3);
				expect(result.input![0].role).toBe('developer');
				expect((result.input![0].content as any)[0].text).toContain('apply_patch');
