...
Complete reference for configuring the OpenHei OpenAI Codex Auth Plugin.
...
```json
{
  ...
  "plugin": ["openhei-openai-codex-auth"],
  "provider": {
    "openai": {
      "options": {
...
openhei run "task" --model=openai/my-custom-id

# TUI shows: "My Display Name"
```

> **⚠️ Recommendation:** Stick to the official presets in `openhei-modern.json` (v1.0.210+) or `openhei-legacy.json` rather than creating custom model variants. GPT 5 models need specific configurations to work reliably.
...
**~/.config/openhei/openhei.jsonc** (global, preferred):
```json
{
  "plugin": ["openhei-openai-codex-auth"],
  "provider": {
...
**my-project/.openhei.json** (project):
```json
{
  "provider": {
    "openai": {
...
Advanced plugin settings in `~/.openhei/openai-codex-auth-config.json`:
...
**What it does:**
- `true` (default): Uses Codex-OpenHei bridge prompt (Task tool & MCP aware)
- `false`: Uses legacy tool remap message
- Bridge prompt content is synced with the latest Codex CLI release (ETag-cached)

**When to disable:**
- Compatibility issues with OpenHei updates
- Testing different prompt styles
- Debugging tool call issues

**Override with environment variable:**
```bash
CODEX_MODE=0 openhei run "task"  # Temporarily disable
CODEX_MODE=1 openhei run "task"  # Temporarily enable
```

### Prompt caching

- When OpenHei provides a `prompt_cache_key` (its session identifier), the plugin forwards it directly to Codex.
...
- Messages bubble up in OpenHei exactly where SDK errors normally surface.
- Helpful when working inside the OpenHei UI or CLI—users immediately see reset timing.
...
**Provided Examples:**
- [config/openhei-modern.json](../config/openhei-modern.json) - Variants-based config for OpenHei v1.0.210+
- [config/openhei-legacy.json](../config/openhei-legacy.json) - Legacy full list for v1.0.209 and below

> **⚠️ REQUIRED:** You MUST use the config that matches your OpenHei version (`openhei-modern.json` or `openhei-legacy.json`). Minimal configs are NOT supported for GPT 5 models and will fail unpredictably. OpenHei's auto-compaction and usage widgets also require the full config's per-model `limit` metadata.

**Your Configs:**
- `~/.config/openhei/openhei.jsonc` - Global config (preferred)
- `~/.config/openhei/openhei.json` - Global config (fallback)
- `<project>/.openhei.json` - Project-specific config
- `~/.openhei/openai-codex-auth-config.json` - Plugin config
...
```bash
# OpenHei will show errors if config is invalid
openhei
```
...
DEBUG_CODEX_PLUGIN=1 openhei run "test" --model=openai/your-model-name
```
...
ENABLE_PLUGIN_REQUEST_LOGGING=1 openhei run "test" --model=openai/gpt-5-codex-low
ENABLE_PLUGIN_REQUEST_LOGGING=1 openhei run "test" --model=openai/gpt-5-codex-high

# Compare reasoning.effort in logs
cat ~/.openhei/logs/codex-plugin/request-*-after-transform.json | jq '.reasoning.effort'
```
...
**⚠️ IMPORTANT:** Old configs with GPT 5.0 models are deprecated. You MUST migrate to the new GPT 5.x configs (`openhei-modern.json` or `openhei-legacy.json`).
...
Use the official config file (`openhei-modern.json` for v1.0.210+, `openhei-legacy.json` for older) which includes:
...
- Proper limit metadata for OpenHei features
...
```
```bash
openhei run "test" --model=openai/my-model  # Must match exactly
```
...
```bash
DEBUG_CODEX_PLUGIN=1 openhei run "test" --model=openai/your-model
```
...
> **⚠️ Best Practice:** Use the official `openhei-modern.json` or `openhei-legacy.json` configuration instead of creating custom configs. This ensures proper model normalization and compatibility with GPT 5 models.
