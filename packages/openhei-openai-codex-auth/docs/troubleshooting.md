...
Common issues and debugging techniques for the OpenHei OpenAI Codex Auth Plugin.
...
cat ~/.openhei/auth/openai.json | jq '.expires'
...
**Symptoms:**
- `openhei auth login` succeeds but no browser window
...
**Solutions:**
- Re-run `openhei auth login` to generate a fresh URL
...
```bash
openhei run "test" --model=openai/gpt-5-codex-low  # Must match config key
```
...
**Check logs:**
```bash
ENABLE_PLUGIN_REQUEST_LOGGING=1 openhei
> first message
> second message
```
...
cat ~/.openhei/logs/codex-plugin/request-*-after-transform.json | jq '.body.input | length'
...
**Check error details:**
```bash
ENABLE_PLUGIN_REQUEST_LOGGING=1 openhei run "test"

# Read error
cat ~/.openhei/logs/codex-plugin/request-*-error-response.json
```
...
```bash
cat ~/.openhei/logs/codex-plugin/request-*-response.json | jq '.headers["x-codex-primary-reset-after-seconds"]'
```

**2. Switch to different model:**
```bash
# If codex is rate limited, try gpt-5
openhei run "task" --model=openai/gpt-5
```
...
```bash
# Exit and restart OpenHei (clears history)
```

**2. Use compact mode** (if OpenHei supports it)
...
ls -lt ~/.openhei/cache/codex-instructions-meta.json

# Check lastChecked timestamp
cat ~/.openhei/cache/codex-instructions-meta.json | jq '.lastChecked'
```
...
DEBUG_CODEX_PLUGIN=1 ENABLE_PLUGIN_REQUEST_LOGGING=1 openhei run "test"
```
...
**Log locations:**
- `~/.openhei/logs/codex-plugin/request-*-before-transform.json`
- `~/.openhei/logs/codex-plugin/request-*-after-transform.json`
- `~/.openhei/logs/codex-plugin/request-*-response.json`
...
cat ~/.openhei/logs/codex-plugin/request-*-after-transform.json | jq '{
  model: .body.model,
...
cat ~/.openhei/logs/codex-plugin/request-*-stream-full.json | grep -o '"total_tokens":[0-9]*'
```
...
1. **Enable logging:**
   ```bash
   DEBUG_CODEX_PLUGIN=1 ENABLE_PLUGIN_REQUEST_LOGGING=1 openhei run "your command"
   ```

2. **Collect info:**
   - OpenHei version: `openhei --version`
   - Plugin version: Check `package.json` or npm
   - Error logs from `~/.openhei/logs/codex-plugin/`
   - Config file (redact sensitive info)

3. **Check existing issues:**
   - [GitHub Issues](https://github.com/numman-ali/openhei-openai-codex-auth/issues)
...
- **Revoke and re-authorize:**
  ...
  2. Remove local tokens: `openhei auth logout`
  3. Re-authenticate: `openhei auth login`
