# OpenHei OpenAI Codex Auth Plugin

> Access GPT-5 Codex through your ChatGPT Plus/Pro subscription in OpenHei

[![npm version](https://badge.fury.io/js/openhei-openai-codex-auth.svg)](https://www.npmjs.com/package/openhei-openai-codex-auth)
...
- [Release Notes](https://github.com/numman-ali/openhei-openai-codex-auth/releases) - Version history and updates
...
Then run OpenHei and authenticate:

```bash
# 1. Add plugin to ~/.config/openhei/openhei.jsonc (or .json)
# 2. Run OpenHei
openhei
...
✅ **Variant system support** - Works with OpenHei v1.0.210+ model variants and legacy presets
...
✅ **Verified Configuration** - Use `config/openhei-modern.json` (v1.0.210+) or `config/openhei-legacy.json` (older)
✅ **Comprehensive Testing** - 200+ unit tests + integration tests

> **⚠️ Important**: GPT 5 models can be temperamental. Use the official config for your OpenHei version (`openhei-modern.json` or `openhei-legacy.json`). Older GPT 5.0 models are deprecated.
...
The plugin intercepts OpenHei's OpenAI SDK requests and transforms them for the ChatGPT backend API:

1. **OAuth Token Management** - Handles token refresh automatically
2. **Request Transformation** - Converts OpenHei SDK format → Codex API format
...
This plugin represents significant engineering effort to bridge OpenHei and the ChatGPT Codex backend:
...
- **Issues**: [GitHub Issues](https://github.com/numman-ali/openhei-openai-codex-auth/issues)
- **Releases**: [Release Notes](https://github.com/numman-ali/openhei-openai-codex-auth/releases)
- **Main Repo**: [GitHub](https://github.com/numman-ali/openhei-openai-codex-auth)
