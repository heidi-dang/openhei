...
This document explains the technical design decisions, architecture, and implementation details of the OpenAI Codex OAuth plugin for OpenHei.
...
**OpenHei/AI SDK sends two incompatible constructs**:
```typescript
// Multi-turn request from OpenHei
const body = {
  input: [
...
5. System Prompt Handling (CODEX_MODE)
   ├─ Filter out OpenHei system prompts
   ├─ Preserve OpenHei env + AGENTS instructions when concatenated
   └─ Add Codex-OpenHei bridge prompt
...
8. Prompt Caching & Session Headers
   ├─ Preserve host-supplied prompt_cache_key (OpenHei session id)
...
|---------|-----------|-------------|------|
| **Codex-OpenHei Bridge** | N/A (native) | ✅ Custom prompt | OpenHei → Codex translation |
| **OpenHei Prompt Filtering** | N/A | ✅ Filter & replace | Remove OpenHei prompts, keep env/AGENTS |
| **Orphan Tool Output Handling** | ✅ Drop orphans | ✅ Convert to messages | Preserve context + avoid 400s |
| **Usage-limit messaging** | CLI prints status | ✅ Friendly error summary | Surface 5h/weekly windows in OpenHei |
| **Per-Model Options** | CLI flags | ✅ Config file | Better UX in OpenHei |
...
### Why Codex-OpenHei Bridge?

**Problem**: OpenHei's system prompts are optimized for OpenHei's tool set and behavior patterns.

**Solution**: Replace OpenHei prompts with Codex-specific instructions.

**Benefits**:
- ✅ Explains tool name differences (apply_patch → edit)
- ✅ Documents available tools
- ✅ Maintains OpenHei working style
- ✅ Preserves Codex best practices
- ✅ 90% reduction in prompt tokens

**Source**: `lib/prompts/codex-openhei-bridge.ts`
...
**Source**: `config/openhei-legacy.json` (legacy) or `config/openhei-modern.json` (variants)
...
**Codex Bridge Prompt**: ~550 tokens (~90% reduction vs full OpenHei prompt)
...
**Source**: `tmp/openhei/packages/openhei/src/provider/transform.ts:90-92`
...
2. **Version Detection**: Adapt to OpenHei/AI SDK version changes
...
2. **OpenHei Changes**: New message ID formats
...
- [OpenHei Source](https://github.com/sst/openhei) - OpenHei implementation
