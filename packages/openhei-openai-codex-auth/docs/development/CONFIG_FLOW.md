# OpenHei Config Flow: Complete Guide

This document explains how OpenHei configuration flows from user files through the plugin system to the Codex API.
...
OpenHei loads and merges configuration from multiple sources in this order (**last wins**):

### 1. Global Config
```
~/.config/openhei/openhei.jsonc
~/.config/openhei/openhei.json
```

### 2. Project Configs (traversed upward from cwd)
```
<project>/.openhei/openhei.jsonc
<project>/.openhei/openhei.json
<parent>/.openhei/openhei.jsonc
<parent>/.openhei/openhei.json
... (up to worktree root)
```

### 3. Custom Config (via flags)
```bash
OPENHEI_CONFIG=/path/to/config.json openhei
# or
OPENHEI_CONFIG_CONTENT='{"model":"openai/gpt-5"}' openhei
```

### 4. Auth Configs
```
# From .well-known/openhei endpoints (for OAuth providers)
https://auth.example.com/.well-known/openhei
```

**Source**: `tmp/openhei/packages/openhei/src/config/config.ts:26-51`
...
**Source**: `tmp/openhei/packages/openhei/src/provider/provider.ts:236-339`
...
**Your Config** (`config/openhei-legacy.json`):
```json
{
  "provider": {
    "openai": {
...
The TUI stores recently used models in `~/.openhei/tui`:
...
**Source**: `tmp/openhei/packages/tui/internal/app/state.go:54-79`
...
❌ **Bad**: `CODEX_MODE=0 openhei`

✅ **Good**: Create `~/.openhei/openai-codex-auth-config.json`:
```json
{
  "codexMode": false
}
...
- [OpenHei Config Schema](https://openhei.ai/config.json) - Official schema
