...
Understanding the difference between config key, `id`, and `name` fields in OpenHei model configuration.
...
- ✅ OpenHei internal lookups: `provider.info.models["gpt-5-codex-low"]`
- ✅ TUI persistence: Saved to `~/.openhei/tui` as `model_id = "gpt-5-codex-low"`
...
**This is the PRIMARY identifier throughout OpenHei!**
...
- ⚠️ **Sorting**: Used for model priority sorting in OpenHei
...
- ❌ **NOT required** (OpenHei defaults it to config key)

**Code Reference:** (`tmp/openhei/packages/openhei/src/provider/provider.ts:252`)
```typescript
...
**OpenAI Custom Loader:** (`tmp/openhei/packages/openhei/src/provider/provider.ts:58-65`)
```typescript
openai: async () => {
  return {
...
**Recommendation:** **Omit the `id` field** for OpenAI provider - it's redundant and creates confusion. OpenHei will auto-set it to the config key.
...
**Code Reference:** (`tmp/openhei/packages/openhei/src/provider/provider.ts:253`)
```typescript
const parsedModel: ModelsDev.Model = {
...
│                                                                 │
│  CLI Usage:                                                     │
│  $ openhei run --model=openai/gpt-5-codex-low                 │
│                                 └──────┬──────┘                 │
│                                   CONFIG KEY                    │
│                                                                 │
│  TUI Display:                                                   │
...
│  1. User Selection                                              │
│     openhei run --model=openai/gpt-5-codex-low                │
│     OpenHei parses: providerID="openai"                        │
│                      modelID="gpt-5-codex-low" ← CONFIG KEY    │
│                                                                 │
│  2. OpenHei Provider Lookup                                    │
...
**Purpose:**
- 🎯 **PRIMARY identifier** - used everywhere in OpenHei
...
- ✅ No redundant fields
- ✅ OpenHei auto-sets `id` to config key

**Why no `id` field?**
- For OpenAI provider, the `id` field is NOT used (custom loader receives config key)
- OpenHei defaults `id` to config key if omitted
...
CONFIG KEY is the real identifier! 👑
  ...
  ├─ Used for persistence (saved to ~/.openhei/tui)
