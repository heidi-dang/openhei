...
### Scenario 1: Default OpenHei Models (No Custom Config)
...
**Available Models:** (from OpenHei's models.dev database)
- `gpt-5`
- `gpt-5-codex`
- `gpt-5-mini`
- `gpt-5-nano`
...
**Test:**
```
User selects: (none - uses OpenHei default)
Plugin receives: undefined or default from OpenHei
...
**Expected:**
- Uses default OpenHei model: `gpt-5-codex`
...
```bash
DEBUG_CODEX_PLUGIN=1 openhei run "test" --model=openai/gpt-5-codex-low
```
...
```bash
DEBUG_CODEX_PLUGIN=1 openhei run "test" --model=openai/gpt-5-codex
```
...
```bash
# 1. Clear cache
(cd ~ && rm -rf .cache/openhei/node_modules/openhei-openai-codex-auth)

# 2. Use minimal config
cat > ~/.config/openhei/openhei.jsonc <<'EOF'
{
  "plugin": ["openhei-openai-codex-auth"],
  "model": "openai/gpt-5-codex"
}
...
DEBUG_CODEX_PLUGIN=1 openhei run "write hello world to test.txt"
```
...
DEBUG_CODEX_PLUGIN=1 openhei run "test low" --model=openai/gpt-5-codex-low
DEBUG_CODEX_PLUGIN=1 openhei run "test high" --model=openai/gpt-5-codex-high
```
...
```bash
DEBUG_CODEX_PLUGIN=1 openhei --model=openai/gpt-5-codex-medium
```
...
```bash
# 1. Start openhei
openhei --model=openai/gpt-5-codex-high
...
```bash
openhei run "test" --model=openai/claude-3.5
```

**Expected:** ❌ Error before plugin (OpenHei rejects unknown model)

### Missing Authentication

```bash
# Without running: openhei auth login
openhei run "test" --model=openai/gpt-5-codex
```
