...
### Added
- **JSONC-aware installer**: preserves comments/formatting and prioritizes `openhei.jsonc` over `openhei.json`.
...
### Changed
- **Default config path**: installer creates `~/.config/openhei/openhei.jsonc` when no config exists.
...
### Added
- **One-command installer/update**: `npx -y openhei-openai-codex-auth@latest` (global config, backup, cache clear) with `--legacy` for OpenHei v1.0.209 and below.
- **Modern variants config**: `config/openhei-modern.json` for OpenHei v1.0.210+; legacy presets remain in `config/openhei-legacy.json`.
...
- **OpenHei prompt source**: updates to the current upstream repository (`anomalyco/openhei`).
...
- **Config presets**: `config/openhei-legacy.json` includes the 22 pre-configured presets (adds GPT 5.2 Codex); `config/openhei-modern.json` provides the variant-based setup.
...
- **Full image input support**: All 16 model variants now include `modalities.input: ["text", "image"]` enabling full multimodal capabilities - read screenshots, diagrams, UI mockups, and any image directly in OpenHei.
...
### Technical Details
- Root cause of compaction issue: OpenHei sends `item_reference` with `fc_*` IDs for function calls. We filter these for stateless mode, but v4.0.1 then removed ALL tool items. Now we only remove orphaned `function_call_output` items (where no matching `function_call` exists).
...
### Fixed
- **Orphaned `function_call_output` errors**: Fixed 400 errors during summary/compaction requests when OpenHei sends `item_reference` pointers to server-stored function calls. The plugin now filters out `function_call` and `function_call_output` items when no tools are present in the request.
...
### Technical Details
- Root cause: OpenHei's secondary model (gpt-5-nano) uses `item_reference` with `fc_*` IDs to reference stored function calls. Our plugin filters `item_reference` for stateless mode (`store: false`), leaving `function_call_output` orphaned. The Codex API rejects requests with orphaned outputs.
...
### Fixed
- Fixed OpenHei prompt cache URL to fetch from `dev` branch instead of non-existent `main` branch.
- Fixed model configuration test script to correctly identify model logs in multi-model sessions (openhei uses a small model like `gpt-5-nano` for title generation alongside the user's selected model).
...
## [3.2.0] - 2025-11-14
### Added
- GPT 5.1 model family support: normalization for `gpt-5.1`, `gpt-5.1-codex`, and `gpt-5.1-codex-mini` plus new GPT 5.1-only presets in the canonical `config/openhei-legacy.json`.
...
- The legacy GPT 5.0 full configuration is now published separately; new installs should prefer the 5.1 presets in `config/openhei-legacy.json`.
...
- Documentation guidance noting that OpenHei's context auto-compaction and usage sidebar require the canonical `config/openhei-legacy.json`.

### Changed
- Prompt caching now relies solely on the host-supplied `prompt_cache_key`; conversation/session headers are forwarded only when OpenHei provides one.
...
## [2.1.0] - 2025-10-04
### Added
- Enhanced CODEX_MODE bridge prompt with Task tool and MCP awareness plus ETag-backed verification of OpenHei system prompts.
...
## [1.0.1] - 2025-10-01
### Added
- README clarifications: openhei auto-installs plugins, config locations, and streamlined quick-start instructions.
