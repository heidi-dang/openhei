...
3. **Request Transformation**:
   ...
   - Filter OpenHei system prompts when in CODEX_MODE
...
- `codex-openhei-bridge.ts`: CODEX_MODE bridge prompt for CLI parity
...
- **Priority**: `CODEX_MODE` env var > `~/.openhei/openai-codex-auth-config.json` > default (true)
- When enabled: Filters out OpenHei system prompts, adds Codex-OpenHei bridge prompt with Task tool & MCP awareness
...
- **Prompt verification**: Caches OpenHei's codex.txt from GitHub (ETag-based) to verify exact prompt removal, with fallback to text signature matching
...
- Input filtering: `filterInput()`, `filterOpenHeiSystemPrompts()`
- Message injection: `addCodexBridgeMessage()` or `addToolRemapMessage()`
- Reasoning config: `getReasoningConfig()` (follows Codex CLI defaults, not openhei defaults)
...
This plugin **intentionally differs from openhei defaults** because it accesses ChatGPT backend API (not OpenAI Platform API):

| Setting | openhei Default | This Plugin Default | Reason |
|---------|-----------------|---------------------|--------|
...
| gpt-5-codex config | (excluded) | Full support | openhei excludes gpt-5-codex from auto-config |
...
- **Plugin config**: `~/.openhei/openai-codex-auth-config.json`
- **Cache dir**: `~/.openhei/cache/`
  ...
  - `openhei-codex.txt` (OpenHei system prompt from GitHub, for verification)
  - `openhei-codex-meta.json` (ETag for OpenHei prompt)
- **Debug logs**: `~/.openhei/logs/codex-plugin/` (when `ENABLE_PLUGIN_REQUEST_LOGGING=1`)
...
**Development**:
- `@openhei-ai/plugin` (peer dependency)
