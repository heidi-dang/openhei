...
import { createOpencode, type ToolPart } from "@openhei-ai/sdk"
...
console.log("🚀 Starting openhei server...")
const openhei = await createOpencode({
  port: 0,
})
...
;(async () => {
  const events = await openhei.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openhei session...")
    const { client, server } = openhei
    ...
    console.log("✅ Created openhei session:", createResult.data.id)
  ...
  console.log("📝 Sending to openhei:", message.text)
