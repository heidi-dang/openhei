...
const { use: useSettingsContext, provider: SettingsProvider, ctx } = createSimpleContext({
    name: "Settings",
    init: () => {
...
/**
 * Safety-net useSettings hook that returns stable default store in production
 * instead of throwing, preventing P0 crashes in the composer.
 */
let lastWarned = 0
export function useSettings() {
    ...
    if (!value) {
        if (import.meta.env.DEV) {
            throw new Error("Settings context must be used within a SettingsProvider")
        }

        // In production, return a no-op store with default values to prevent crashes.
        ...
        return {
            ...
            general: {
                autoSave: () => DEFAULT_SETTINGS.general.autoSave,
                setAutoSave: () => { },
                releaseNotes: () => DEFAULT_SETTINGS.general.releaseNotes,
                setReleaseNotes: () => { },
                showReasoningSummaries: () => DEFAULT_SETTINGS.general.showReasoningSummaries,
                setShowReasoningSummaries: () => { },
                thinkingDrawerMode: () => DEFAULT_SETTINGS.general.thinkingDrawerMode,
                setThinkingDrawerMode: () => { },
            },
            ml: {
                qloraEnabled: () => DEFAULT_SETTINGS.ml.qloraEnabled,
                setQLoRAEnabled: () => { },
            },
            updates: {
                startup: () => DEFAULT_SETTINGS.updates.startup,
                setStartup: () => { },
            },
            appearance: {
                fontSize: () => DEFAULT_SETTINGS.appearance.fontSize,
                setFontSize: () => { },
                font: () => DEFAULT_SETTINGS.appearance.font,
                setFont: () => { },
            },
            keybinds: {
                get: (action: string) => DEFAULT_SETTINGS.keybinds[action],
                set: () => { },
                reset: () => { },
                resetAll: () => { },
            },
            permissions: {
                autoApprove: () => DEFAULT_SETTINGS.permissions.autoApprove,
                setAutoApprove: () => { },
            },
            flags: {
                get: <K extends keyof Settings["flags"]>(key: K) => DEFAULT_SETTINGS.flags[key],
                set: () => { },
            },
            notifications: {
                agent: () => DEFAULT_SETTINGS.notifications.agent,
                setAgent: () => { },
                permissions: () => DEFAULT_SETTINGS.notifications.permissions,
                setPermissions: () => { },
                errors: () => DEFAULT_SETTINGS.notifications.errors,
                setErrors: () => { },
            },
            sounds: {
                agentEnabled: () => DEFAULT_SETTINGS.sounds.agentEnabled,
                setAgentEnabled: () => { },
                agent: () => DEFAULT_SETTINGS.sounds.agent,
                setAgent: () => { },
                permissionsEnabled: () => DEFAULT_SETTINGS.sounds.permissionsEnabled,
                setPermissionsEnabled: () => { },
                permissions: () => DEFAULT_SETTINGS.sounds.permissions,
                setPermissions: () => { },
                errorsEnabled: () => DEFAULT_SETTINGS.sounds.errorsEnabled,
                setErrorsEnabled: () => { },
                errors: () => DEFAULT_SETTINGS.sounds.errors,
                setErrors: () => { },
            },
