---
title: Server
description: Interact with openhei server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openhei serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openhei client can use.
...
Set `OPENHEI_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openhei`, or set `OPENHEI_SERVER_USERNAME` to override it. This applies to both `openhei serve` and `openhei web`.
...
When you run `openhei` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openhei server to interact with openhei programmatically.
:::

This architecture lets openhei support multiple clients and allows you to interact with openhei programmatically.

You can run `openhei serve` to start a standalone server. If you have the
openhei TUI running, `openhei serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenHei [IDE](/docs/ide) plugins.
...
The openhei server exposes the following APIs.
