...
There are two types of agents in OpenHei; primary agents and subagents.
...
OpenHei comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
OpenHei comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

OpenHei comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `openhei.json` config file:
...
If no temperature is specified, OpenHei uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global OpenHei config and the project specific config.
...
The model ID in your OpenHei config uses the format `provider/model-id`. For example, if you're using [OpenHei Zen](/docs/zen), you would use `openhei/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `openhei models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/heidi-dang/openhei).
:::
