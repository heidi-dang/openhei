---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en OpenHei.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHei utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration OpenHei.
...
en `~/.local/share/openhei/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre OpenHei
configuration.
...
OpenHei Zen est une liste de modèles fournis par l'équipe OpenHei qui ont été
testé et vérifié pour fonctionner correctement avec OpenHei. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par OpenHei Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez openhei et dirigez-vous vers [openhei.ai/auth](https://openhei.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans OpenHei et son utilisation est totalement facultative.
...
Pour utiliser Amazon Bedrock avec OpenHei :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de openhei :
...
Pour une configuration spécifique au projet ou persistante, utilisez `openhei.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans OpenHei n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que openhei fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration openhei.
...
pour stocker le jeton dans le stockage d'authentification openhei.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
OpenHei utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller OpenHei
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `openhei.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuration

Personnalisez via `openhei.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec openhei :
...
Pour utiliser Google Vertex AI avec OpenHei :
...
Définissez-les lors de l'exécution de openhei.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via openhei, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) pour enregistrer automatiquement chaque conversation OpenHei en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation OpenHei répertoriée comme une session distincte.
...
Vous pouvez configurer openhei pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer openhei pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer openhei pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenHei. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/openhei) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec OpenHei :
...
3. Copiez la clé API à utiliser dans OpenHei.
...
6. **Important** : Avant d'utiliser des modèles cloud dans OpenHei, vous devez extraire les informations du modèle localement :
...
OpenHei Zen est une liste de modèles testés et vérifiés fournie par l'équipe OpenHei. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>OpenHei Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **OpenHei Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé OpenHei API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration openhei.
...
5. Vous pouvez également les personnaliser via votre configuration openhei. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration openhei. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec openhei. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `openhei.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à OpenHei de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `openhei auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration openhei et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration openhei.
