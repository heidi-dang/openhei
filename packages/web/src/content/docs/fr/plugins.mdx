---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenHei.
---

Les plugins vous permettent d'étendre OpenHei en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenHei.
...
- `.openhei/plugins/` - Plugins au niveau du projet
- `~/.config/openhei/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openhei/node_modules/`.
...
1. Configuration globale (`~/.config/openhei/openhei.json`)
2. Configuration du projet (`openhei.json`)
3. Répertoire global des plugins (`~/.config/openhei/plugins/`)
4. Répertoire des plugins du projet (`.openhei/plugins/`)
...
OpenHei exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK openhei pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openhei.
...
:::note
Si vous utilisez l'application de bureau OpenHei, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez openhei de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openhei :
...
L'assistant `tool` crée un outil personnalisé que openhei peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openhei aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://openhei.ai/docs/sdk) pour plus de détails.
