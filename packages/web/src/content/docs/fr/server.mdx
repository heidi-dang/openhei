---
title: Serveur
description: Interagissez avec le serveur openhei via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openhei serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openhei peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `openhei.local` |
...
Définissez `OPENHEI_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openhei`, ou définissez `OPENHEI_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openhei serve` et à `openhei web`.
...
Lorsque vous exécutez `openhei`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openhei pour interagir avec openhei par programmation.
:::

Cette architecture permet à openhei de prendre en charge plusieurs clients et vous permet d'interagir avec openhei par programme.

Vous pouvez exécuter `openhei serve` pour démarrer un serveur autonome. Si vous avez le
openhei TUI en cours d'exécution, `openhei serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenHei [IDE](/docs/ide).
...
Le serveur openhei expose les API suivantes.
