...
description: Créez des outils que le LLM peut appeler dans openhei.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de openhei comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.openhei/tools/` de votre projet.
- Ou globalement, en les plaçant dans `~/.config/openhei/tools/`.
...
```ts title=".openhei/tools/database.ts" {1}
...
```ts title=".openhei/tools/math.ts"
import { tool } from "@openhei-ai/plugin"
...
```ts title=".openhei/tools/project.ts" {8}
import { tool } from "@openhei-ai/plugin"
...
```python title=".openhei/tools/add.py"
import sys
...
```ts title=".openhei/tools/python-add.ts" {10}
import { tool } from "@openhei-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openhei/tools/add.py")
