...
Il existe deux types d'agents dans OpenHei ; agents primaires et sous-agents.
...
OpenHei est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
OpenHei est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

OpenHei est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `openhei.json` :
...
- Global : `~/.config/openhei/agents/`
- Par projet : `.openhei/agents/`

```markdown title="~/.config/openhei/agents/review.md"
---
...
Si aucune température n'est spécifiée, OpenHei utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale OpenHei et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration OpenHei utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenHei Zen](/docs/zen), vous utiliserez `openhei/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="openhei.json" {3-6,9-12}
{
  "$schema": "https://openhei.ai/config.json",
  "tools": {
    "write": true,
...
```json title="openhei.json" {3-5,8-10}
{
  "$schema": "https://openhei.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `openhei models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/heidi-dang/openhei).
:::
...
```markdown title="~/.config/openhei/agents/docs-writer.md"
---
...
```markdown title="~/.config/openhei/agents/security-auditor.md"
---
