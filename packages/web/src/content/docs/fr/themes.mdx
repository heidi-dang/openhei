...
Avec OpenHei, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenHei utilise notre propre thème `openhei`.
...
OpenHei est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenHei corresponde à l'apparence de leur terminal
...
OpenHei prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openhei/themes/*.json` ou `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openhei/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openhei/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/openhei/themes
vim ~/.config/openhei/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .openhei/themes
vim .openhei/themes/my-theme.json
```
...
{
  "$schema": "https://openhei.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
