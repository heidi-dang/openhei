---
title: Providers
description: Using any LLM provider in OpenHei.
---

import config from "../../../config.mjs"
export const console = config.console

OpenHei uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your OpenHei config.
...
You can customize the providers through the `provider` section in your OpenHei
config.
...
OpenHei Zen is a list of models provided by the OpenHei team that have been
tested and verified to work well with OpenHei. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with OpenHei Zen.
:::

1. Run the `/connect` command in the TUI, select openhei, and head to [openhei.ai/auth](https://openhei.ai/auth).
...
It works like any other provider in OpenHei and is completely optional to use.
...
To use Amazon Bedrock with OpenHei:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running openhei:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `openhei.json`:
...
:::info
Using your Claude Pro/Max subscription in OpenHei is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for openhei to work properly.
   :::
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your openhei config.
...
OpenHei uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock OpenHei
to only use your own GitLab-hosted instance, add the following to your
`openhei.json` file. It is also recommended to disable session sharing.
...
More documentation on [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) homepage.
...
To use your GitHub Copilot subscription with openhei:
...
To use Google Vertex AI with OpenHei:
...
In the event you see a feature or model from Helicone that isn't configured automatically through openhei, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) plugin to automatically log each OpenHei conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each OpenHei conversation listed as a separate session.
...
You can configure openhei to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure openhei to use local models through LM Studio.
...
You can configure openhei to use local models through Ollama.

:::tip
Ollama can automatically configure itself for OpenHei. See the [Ollama integration docs](https://docs.ollama.com/integrations/openhei) for details.
:::
...
To use Ollama Cloud with OpenHei:
...
3. Copy the API key for use in OpenHei.
...
6. **Important**: Before using cloud models in OpenHei, you must pull the model information locally:
...
OpenHei Zen is a list of tested and verified models provided by the OpenHei team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>OpenHei Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **OpenHei Zen**.

   ```txt
   /connect
   ```

3. Enter your OpenHei API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your openhei config.
...
5. You can also customize them through your openhei config. Here's an example of specifying a provider
...
You can also customize models through your openhei config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with openhei. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
4. Create or update your `openhei.json` file in your project directory:
...
The `limit` fields allow OpenHei to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `openhei auth list` to see if the credentials
...
2. For custom providers, check the openhei config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your openhei config.
