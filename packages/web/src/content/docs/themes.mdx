...
With OpenHei you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenHei uses our own `openhei` theme.
...
OpenHei comes with several built-in themes.
...
OpenHei supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/openhei/themes/*.json` or `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openhei/themes/*.json`
4. **Current working directory** - Defined in `./.openhei/themes/*.json`
