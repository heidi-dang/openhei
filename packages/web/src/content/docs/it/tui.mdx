---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di OpenHei.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenHei fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire OpenHei avvia la TUI per la directory corrente.
...
Quando usi la TUI di OpenHei, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a OpenHei. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da OpenHei. _Alias_: `/quit`, `/q`
...
Puoi personalizzare il comportamento della TUI tramite il file di config di OpenHei.
