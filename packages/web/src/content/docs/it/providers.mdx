---
title: Provider
description: Usa qualsiasi provider LLM in OpenHei.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHei usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di OpenHei.
...
OpenHei Zen e una lista di modelli forniti dal team OpenHei, testati e
verificati per funzionare bene con OpenHei. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da OpenHei Zen.
:::

1. Esegui `/connect` nella TUI, seleziona openhei e vai su [openhei.ai/auth](https://openhei.ai/auth).
...
Funziona come qualsiasi altro provider in OpenHei ed e totalmente opzionale.
...
Per usare Amazon Bedrock con OpenHei:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui openhei:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `openhei.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in OpenHei non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché openhei funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di openhei.
...
salvare il token nello storage di auth di openhei.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
OpenHei usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare OpenHei
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `openhei.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configurazione

Personalizza tramite `openhei.json`:
...
Per usare il tuo abbonamento GitHub Copilot con openhei:
...
Per usare Google Vertex AI con OpenHei:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui openhei.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da openhei, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) per loggare automaticamente ogni conversazione di OpenHei come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di OpenHei elencata come sessione separata.
...
Puoi configurare openhei per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare openhei per usare modelli locali tramite LM Studio.
...
Puoi configurare openhei per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per OpenHei. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/openhei) per dettagli.
:::
...
Per usare Ollama Cloud con OpenHei:
...
3. Copia la chiave API da usare in OpenHei.
...
6. **Importante**: Prima di usare modelli cloud in OpenHei, devi scaricare le informazioni del modello localmente:
...
OpenHei Zen è una lista di modelli testati e verificati forniti dal team OpenHei. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>OpenHei Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **OpenHei Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di OpenHei.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di openhei.
...
5. Puoi anche personalizzarli tramite la configurazione di openhei. Ecco un esempio di specifica di un provider
...
Puoi anche personalizzare i modelli tramite la tua configurazione di openhei. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con openhei. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `openhei.json` nella directory del progetto:
...
I campi `limit` permettono a OpenHei di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `openhei auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di openhei e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config openhei.
