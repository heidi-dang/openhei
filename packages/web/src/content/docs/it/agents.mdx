...
In OpenHei esistono due tipi di agenti: agenti primari e subagenti.
...
OpenHei include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
OpenHei include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

OpenHei include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `openhei.json`:
...
- Globale: `~/.config/openhei/agents/`
- Per progetto: `.openhei/agents/`
...
Se non specifichi una temperature, OpenHei usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di OpenHei sia per la config specifica del progetto.
...
L'ID modello nella configurazione OpenHei usa il formato `provider/model-id`. Per esempio, se stai usando [OpenHei Zen](/docs/zen), useresti `openhei/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `openhei models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/heidi-dang/openhei).
:::
