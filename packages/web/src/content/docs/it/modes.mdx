...
:::caution
Le modalita ora si configurano tramite l'opzione `agent` nella configurazione di openhei. L'opzione
`mode` e ora deprecata. [Scopri di piu](/docs/agents).
:::

Le modalita in openhei ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.

Include due modalita integrate: **build** e **plan**. Puoi personalizzarle oppure configurarne di tue tramite la configurazione di openhei.
...
openhei include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.openhei/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `openhei.json`:
...
- Globale: `~/.config/openhei/modes/`
- Progetto: `.openhei/modes/`
...
Se non viene specificata alcuna temperatura, openhei usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di openhei sia per quella specifica del progetto.
...
Crea file di modalita in `.openhei/modes/` per modalita specifiche del progetto oppure in `~/.config/openhei/modes/` per modalita globali:
