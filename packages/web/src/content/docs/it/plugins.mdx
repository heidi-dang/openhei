---
title: Plugin
description: Scrivi plugin per estendere OpenHei.
---

I plugin ti permettono di estendere OpenHei agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalita', integrare servizi esterni o modificare il comportamento predefinito di OpenHei.
...
- `.openhei/plugins/` - plugin a livello progetto
- `~/.config/openhei/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openhei/node_modules/`.
...
1. Config globale (`~/.config/openhei/openhei.json`)
2. Config di progetto (`openhei.json`)
3. Directory plugin globale (`~/.config/openhei/plugins/`)
4. Directory plugin di progetto (`.openhei/plugins/`)
...
OpenHei esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di openhei per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openhei.
...
:::note
Se usi l'app desktop di OpenHei, puo' inviare automaticamente notifiche di sistema quando una risposta e' pronta o quando una sessione va in errore.
:::
...
Impedisci a openhei di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openhei:
...
L'helper `tool` crea uno strumento personalizzato che openhei puo' chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openhei insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://openhei.ai/docs/sdk) per i dettagli.
