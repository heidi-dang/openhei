---
title: Server
description: Interagisci con il server openhei via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `openhei serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client openhei.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `openhei.local` |
...
Imposta `OPENHEI_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `openhei`, oppure imposta `OPENHEI_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `openhei serve` sia per `openhei web`.
...
Quando esegui `openhei` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server openhei per interagire con openhei in modo programmatico.
:::

Questa architettura permette a openhei di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `openhei serve` per avviare un server standalone. Se la TUI di openhei e gia in esecuzione, `openhei serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OpenHei.
...
Il server openhei espone le seguenti API.
