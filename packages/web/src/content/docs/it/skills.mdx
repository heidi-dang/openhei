...
Le skill degli agenti permettono a OpenHei di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenHei cerca in queste posizioni:

- Config di progetto: `.openhei/skills/<name>/SKILL.md`
- Config globale: `~/.config/openhei/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenHei risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.openhei/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/openhei/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.openhei/skills/git-release/SKILL.md` cosi':
...
OpenHei elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `openhei.json`:
...
**Per agenti integrati** (in `openhei.json`):
