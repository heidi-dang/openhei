...
Agent Skills erlauben OpenHei, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenHei sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenHei vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.openhei/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/openhei/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.openhei/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenHei listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `openhei.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `openhei.json`):
