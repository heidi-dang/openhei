---
title: Server
description: Nutze den openhei-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `openhei serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein openhei-Client nutzen kann.
...
Setze `OPENHEI_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `openhei`, kann aber mit `OPENHEI_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `openhei serve` und `openhei web`.
...
Wenn du `openhei` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den openhei-Server, um openhei programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `openhei serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `openhei serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenHei-[IDE](/docs/ide)-Plugins.
...
Der openhei-Server stellt folgende APIs bereit.
