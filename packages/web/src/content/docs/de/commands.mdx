...
Erstellen Sie `.openhei/commands/test.md`:

```md title=".openhei/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die OpenHei-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem OpenHei [config](/docs/config):

```json title="openhei.jsonc" {4-12}
{
...
- Global: `~/.config/openhei/commands/`
- Pro Projekt: `.openhei/commands/`

```markdown title="~/.config/openhei/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".openhei/commands/component.md"
---
...
```md title=".openhei/commands/create-file.md"
---
...
```md title=".openhei/commands/analyze-coverage.md"
---
...
```md title=".openhei/commands/review-changes.md"
---
...
```md title=".openhei/commands/review-component.md"
---
...
OpenHei enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
