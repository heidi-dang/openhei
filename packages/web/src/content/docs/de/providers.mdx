---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in OpenHei.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHei verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer OpenHei-Konfiguration.
...
in `~/.local/share/openhei/auth.json`.

---

### Konfiguration

Sie können die Anbieter über den Abschnitt `provider` in Ihrem OpenHei anpassen
config.
...
OpenHei Zen ist eine Liste von Modellen, die vom OpenHei-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit OpenHei funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit OpenHei Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OpenHei aus und gehen Sie zu [openhei.ai/auth](https://openhei.ai/auth).
...
Es funktioniert wie jeder andere Anbieter in OpenHei und ist völlig optional.
...
So verwenden Sie Amazon Bedrock mit OpenHei:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie OpenHei ausführen:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY openhei

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile openhei

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX openhei
   ```
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `openhei.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in OpenHei wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit OpenHei ordnungsgemäß funktioniert.
   :::
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX openhei
   ```
...
5. Legen Sie Ihren Ressourcennamen als Umgebungsvariable fest:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX openhei
   ```
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre OpenHei-Konfiguration hinzufügen.
...
um das Token im OpenHei-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
OpenHei verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von OpenHei
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`openhei.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `openhei.json`:
...
{
  "$schema": "https://openhei.ai/config.json",
  "plugin": ["@gitlab/openhei-gitlab-plugin"]
}
```
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit OpenHei:
...
So verwenden Sie Google Vertex AI mit OpenHei:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie OpenHei ausführen.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id openhei
   ```
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über OpenHei konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
```jsonc title="~/.config/openhei/openhei.jsonc"
{
  "$schema": "https://openhei.ai/config.json",
  "provider": {
    "helicone": {
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session)-Plugin, um jede OpenHei-Konversation automatisch als Sitzung in Helicone zu protokollieren.

```bash
npm install -g openhei-helicone-session
```
...
```json title="openhei.json"
{
  "plugin": ["openhei-helicone-session"]
}
```

Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede OpenHei-Konversation als separate Sitzung aufgeführt.
...
Sie können OpenHei für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren

```json title="openhei.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://openhei.ai/config.json",
  "provider": {
    "llama.cpp": {
...
Sie können OpenHei über LM Studio für die Verwendung lokaler Modelle konfigurieren.

```json title="openhei.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://openhei.ai/config.json",
  "provider": {
    "lmstudio": {
...
Sie können OpenHei über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für OpenHei konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/openhei).
:::

```json title="openhei.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://openhei.ai/config.json",
  "provider": {
    "ollama": {
...
So verwenden Sie Ollama Cloud mit OpenHei:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in OpenHei.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in OpenHei verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
OpenHei Zen ist eine Liste getesteter und verifizierter Modelle, die vom OpenHei-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>OpenHei Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **OpenHei Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren OpenHei API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre OpenHei-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre OpenHei-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
3. Geben Sie Ihren JSON-Service-Account-Key ein.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' openhei
   ```
...
4. Legen Sie optional die Bereitstellung ID und die Ressourcengruppe fest:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group openhei
   ```
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit OpenHei:
...
Sie können Modelle auch über Ihre OpenHei-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit OpenHei verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
3. Geben Sie Ihren API-Schlüssel für den Anbieter ein.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in openhei.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Erstellen oder aktualisieren Sie Ihre `openhei.json`-Datei in Ihrem Projektverzeichnis:

   ```json title="openhei.json" ""myprovider"" {5-15}
   {
     "$schema": "https://openhei.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="openhei.json" {9,11,17-20}
{
  "$schema": "https://openhei.ai/config.json",
  "provider": {
    "myprovider": {
...
Mithilfe der `limit`-Felder kann OpenHei erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `openhei auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die OpenHei-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer OpenHei-Konfiguration übereinstimmt.
