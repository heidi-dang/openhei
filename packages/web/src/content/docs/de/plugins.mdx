---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenHei zu erweitern.
---

Mit Plugins können Sie OpenHei erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenHei zu ändern.
...
- `.openhei/plugins/` – Plugins auf Projektebene
- `~/.config/openhei/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/openhei/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/openhei/openhei.json`)
2. Projektkonfiguration (`openhei.json`)
3. Globales Plugin-Verzeichnis (`~/.config/openhei/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.openhei/plugins/`)
...
OpenHei führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenHei-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenHei erweitern können.
...
:::note
Wenn Sie die OpenHei-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenHei `.env`-Dateien liest:
...
Plugins können OpenHei auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenHei aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenHei zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://openhei.ai/docs/sdk).
