---
title: Regeln
description: Lege eigene Anweisungen fuer openhei fest.
---

Du kannst openhei ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openhei den Befehl `/init` aus.
...
So kann openhei sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openhei sie zu erweitern.
...
openhei kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openhei/AGENTS.md` speichern.
Diese gelten in allen openhei-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenHei dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openhei/AGENTS.md` existiert)
...
Beim Start sucht openhei in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openhei/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openhei.json` oder in `~/.config/openhei/openhei.json` angeben.
...
openhei parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit openhei.json

Empfohlen ist das `instructions`-Feld in der `openhei.json`:
...
Du kannst openhei explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openhei.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
