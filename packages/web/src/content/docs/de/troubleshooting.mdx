...
Wenn OpenHei Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `openhei --log-level DEBUG`.

---

## Speicher

openhei speichert Sitzungs- und App-Daten auf der Festplatte unter:
...
OpenHei Desktop startet im Hintergrund einen lokalen OpenHei-Server (`openhei-cli`-Sidecar).
...
- Nur macOS: `OpenHei`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
Wenn du Probleme mit OpenHei hast:
