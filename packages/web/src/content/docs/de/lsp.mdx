---
title: LSP Server
description: OpenHei lässt sich in Ihren LSP-Server integrieren.
---

OpenHei lässt sich in Ihr Language Server Protocol (LSP) integrieren, um die Interaktion von LLM mit Ihrer Codebasis zu unterstützen. Es verwendet Diagnosefunktionen, um dem LLM Feedback zu geben.

---

## Integriert

OpenHei verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `OPENHEI_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn OpenHei eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer OpenHei-Konfiguration anpassen.
