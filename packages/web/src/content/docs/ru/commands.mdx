...
Создайте `.openhei/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию openhei или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openhei [config](/docs/config):
...
- Глобальный: `~/.config/openhei/commands/`
- Для каждого проекта: `.openhei/commands/`
...
openhei включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
