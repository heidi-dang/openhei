...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenHei dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenHei.
...
:::note
Ovaj alat je dostupan samo kada je `OPENHEI_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENHEI_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenHei provajdera ili kada je varijabla `OPENHEI_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenHei:
