...
Možete dodati vanjske alate u OpenHei koristeći _Model Context Protocol_, ili MCP. OpenHei podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenHei Config](https://openhei.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/openhei`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenHei automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenHei će:
...
Ako server zahtijeva autentifikaciju, OpenHei će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `openhei mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenHei sigurno čuva tokene u `~/.local/share/openhei/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenHei, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenHei konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenHei sa vašim Sentry nalogom.
