---
title: Server
description: Komunicirajte s openhei serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openhei serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openhei klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `openhei.local` |
...
Postavite `OPENHEI_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openhei`, ili postavite `OPENHEI_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openhei serve` i za `openhei web`.
...
Kada pokrenete `openhei`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openhei server za programsku interakciju sa openhei-om.
:::

Ova arhitektura omogucava openhei podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openhei serve` da startate standalone server. Ako je openhei TUI vec pokrenut, `openhei serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenHei [IDE](/docs/ide) pluginovi.
...
openhei server izlaže sljedece API-je.
