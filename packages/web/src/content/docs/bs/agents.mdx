...
Postoje dvije vrste agenata u OpenHei: primarni agenti i podagenti.
...
OpenHei dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
OpenHei dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

OpenHei dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `openhei.json`:
...
- Globalno: `~/.config/openhei/agents/`
- Po projektu: `.openhei/agents/`
...
Ako temperatura nije navedena, OpenHei koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu OpenHei konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj OpenHei konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenHei Zen](/docs/zen), koristili biste `openhei/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `openhei models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/heidi-dang/openhei).
:::
