...
ove ili konfigurirajte svoje putem openhei config.
...
openhei dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.openhei/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `openhei.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/openhei/modes/`
- Projekat: `.openhei/modes/`
...
Ako temperatura nije navedena, openhei koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.openhei/modes/` za specifične načine rada ili `~/.config/openhei/modes/` za globalne načine:
