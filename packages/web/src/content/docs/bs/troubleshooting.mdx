...
Da biste otklonili probleme s OpenHei, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openhei\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `openhei --log-level DEBUG`.

---

## Pohrana

OpenHei pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/openhei/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openhei`
...
OpenHei Desktop pokreće lokalni OpenHei server (`openhei-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenHei` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/openhei/openhei.jsonc` (ili `~/.config/openhei/openhei.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/openhei/openhei.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openhei\openhei.jsonc`
...
OpenHei također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/openhei/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openhei\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenHei mogao ponovo izgraditi.

1. Potpuno zatvorite OpenHei Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/openhei`
- **Linux**: obrišite `~/.cache/openhei` (ili pokrenite `rm -rf ~/.cache/openhei`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\openhei`

3. Ponovo pokrenite OpenHei Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenHei Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `openhei.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENHEI_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENHEI_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OpenHei Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenHei karakteristikama.

---

### Obavještenja se ne prikazuju

OpenHei Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenHei u postavkama vašeg OS-a, i
...
1. Zatvorite OpenHei Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenHei Desktop aplikacije):

- `openhei.settings.dat` (URL zadanog servera za desktop)
- `openhei.global.dat` i `openhei.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `openhei models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\openhei`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenHei dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\openhei`

2. Ponovo pokrenite OpenHei da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenHei da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenHei će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
