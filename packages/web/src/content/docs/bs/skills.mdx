...
Agent skills omogucavaju OpenHei da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenHei pretrazuje ove lokacije:

- Konfiguracija projekta: `.openhei/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/openhei/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenHei ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.openhei/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/openhei/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.openhei/skills/git-release/SKILL.md` ovako:
...
OpenHei navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `openhei.json`:
...
**Za ugradene agente** (u `openhei.json`):
