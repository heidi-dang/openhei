...
U OpenHei mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenHei koristi nasu `openhei` temu.
...
OpenHei dolazi sa vise ugradenih tema.
...
- Zele da OpenHei odgovara izgledu njihovog terminala
...
OpenHei podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/openhei/themes/*.json` ili `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openhei/themes/*.json`
4. **Trenutni radni direktorij** - `./.openhei/themes/*.json`
