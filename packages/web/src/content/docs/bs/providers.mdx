---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenHei.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHei koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenHei konfiguraciji.
...
u `~/.local/share/openhei/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenHei
config.
...
OpenHei Zen je lista modela koje je obezbedio OpenHei tim koji su bili
testirano i potvrđeno da dobro radi sa OpenHei. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenHei Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite openhei i idite na [openhei.ai/auth](https://openhei.ai/auth).
...
Radi kao i svaki drugi provajder u OpenHei i potpuno je opcionalan za korištenje.
...
Da biste koristili Amazon Bedrock s OpenHei:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete openhei:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openhei.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenHei.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi openhei ispravno radio.
   :::
...
Također možete dodati modele kroz svoju openhei konfiguraciju.
...
da pohrani token u openhei auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenHei koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenHei
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`openhei.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `openhei.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s openhei:
...
Za korištenje Google Vertex AI s OpenHei:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće openhei.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem openheia, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) da automatski prijavite svaki OpenHei razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenHei razgovor naveden kao zasebna sesija.
...
Možete konfigurirati openhei za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati openhei za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati openhei za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenHei. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/openhei) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenHei:
...
3. Kopirajte API ključ za korištenje u OpenHei.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenHei, morate lokalno povući informacije o modelu:
...
OpenHei Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenHei tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenHei Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenHei Zen**.

```txt
   /connect
```

3. Unesite svoj OpenHei API ključ.
...
Također možete dodati dodatne modele putem vaše openhei konfiguracije.
...
5. Također ih možete prilagoditi putem vaše openhei konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju openhei konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s openhei-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `openhei.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenHei da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `openhei auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenHei konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj openhei konfiguraciji.
