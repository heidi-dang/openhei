---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenHei.
---

Dodaci vam omogućavaju da proširite OpenHei spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenHei.
...
- `.openhei/plugins/` - Dodaci na nivou projekta
- `~/.config/openhei/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openhei/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openhei/openhei.json`)
2. Konfiguracija projekta (`openhei.json`)
3. Globalni direktorij dodataka (`~/.config/openhei/plugins/`)
4. Direktorij dodataka projekta (`.openhei/plugins/`)
...
OpenHei pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenHei.
...
:::note
Ako koristite desktop aplikaciju OpenHei, ona može automatski slati sistemske obavijesti kada je odgovor spreman ili kada dođe do greške u sesiji.
:::
...
Spriječite openhei da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openhei:
...
Pomoćnik `tool` kreira prilagođeni alat koji openhei može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openhei zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://openhei.ai/docs/sdk) za detalje.
