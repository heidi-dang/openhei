...
openhei'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
openhei, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
openhei, **General** ve **Explore** olmak üzere iki yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

openhei iki yerleşik birincil agent ve iki yerleşik alt agent ile birlikte gelir.
...
Agent'ları `openhei.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse openhei modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global openhei yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
openhei hesabınızdaki model kimliğini `provider/model-id` biçimini kullanır. Örneğin, [OpenHei Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `openhei/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `openhei models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/heidi-dang/openhei).
:::
