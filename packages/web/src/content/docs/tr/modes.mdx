...
:::caution
Modlar artık openhei yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

openhei'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya openhei yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
openhei iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.openhei/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`openhei.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, openhei modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global openhei yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.openhei/modes/` veya genel modlar için `~/.config/openhei/modes/`'de mod dosyaları oluşturun:
