...
openhei ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openhei kendi `openhei` temasını kullanır.
...
openhei birden fazla yerleşik temayla gelir.
...
- openhei'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openhei, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openhei/themes/*.json` veya `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openhei/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openhei/themes/*.json`
