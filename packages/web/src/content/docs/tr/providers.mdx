---
title: Sağlayıcılar
description: openhei'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

openhei, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı openhei yapılandırmanızda yapılandırın.
...
`~/.local/share/openhei/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları openhei'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenHei Zen, openhei ekibi tarafından sağlanan modellerin bir listesidir.
openhei ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenHei Zen ile başlamanızı öneririz.
:::
...
openhei'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.
...
Amazon Bedrock'u openhei ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   openhei'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `openhei.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi openhei'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   openhei'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in openhei auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
openhei, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. openhei'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`openhei.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`openhei.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi openhei'la kullanmak için:
...
Google Vertex AI'yi openhei ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları openhei çalıştırılırken ayarlayın.
...
Helicone'un openhei aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her openhei görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her openhei görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
openhei'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla openhei'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla openhei'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini openhei için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/openhei)'a bakın.
:::
...
Ollama Cloud'u openhei ile kullanmak için:
...
3. openhei'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: openhei'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenHei Zen, openhei ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>OpenHei Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **OpenHei Zen**'i arayın.

   ```txt
   /connect
   ```

3. openhei API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   openhei yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca openhei yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri openhei yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı openhei ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `openhei.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları openhei'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `openhei auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için openhei yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, openhei yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
