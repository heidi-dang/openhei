---
title: Eklentiler
description: openhei'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek openhei'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya openhei'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.openhei/plugins/` - Proje düzeyinde eklentiler
- `~/.config/openhei/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/openhei/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/openhei/openhei.json`)
2. Project config (`openhei.json`)
3. Global eklenti dizini (`~/.config/openhei/plugins/`)
4. Proje eklenti dizini (`.openhei/plugins/`)
...
openhei bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşime geçmek için openhei'lu bir SDK istemcisi.
...
openhei'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
openhei masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
openhei'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca openhei'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, openhei'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK documentation](https://openhei.ai/docs/sdk)'e bakın.
