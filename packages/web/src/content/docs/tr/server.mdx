---
title: Sunucu
description: openhei sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`openhei serve` komutu, openhei istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `openhei.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `OPENHEI_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `openhei` değeridir; değiştirmek isterseniz `OPENHEI_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `openhei serve` hem de `openhei web` için geçerlidir.
...
`openhei` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
openhei ile programatik etkileşim için openhei sunucusunu kullanın.
:::

Bu mimari, openhei'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`openhei serve` ile bağımsız bir sunucu başlatabilirsiniz. openhei TUI açıksa, `openhei serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum openhei [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
openhei sunucusu aşağıdaki API'leri sunar.
