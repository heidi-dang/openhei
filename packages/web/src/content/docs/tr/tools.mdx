...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. openhei yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
openhei içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OPENHEI_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENHEI_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca openhei provider kullanılırken veya `OPENHEI_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

openhei başlatırken etkinleştirmek için:
