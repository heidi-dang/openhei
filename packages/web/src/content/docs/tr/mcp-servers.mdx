...
_Model Bağlam Protokolü_ veya MCP kullanarak openhei'a harici araçlar ekleyebilirsiniz. openhei hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [openhei Config](https://openhei.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/openhei` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
openhei, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openhei şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openhei, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openhei mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openhei, belirteçleri `~/.local/share/openhei/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra openhei'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openhei yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve openhei'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
