---
title: サーバー
description: HTTP 経由で OpenHei サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`openhei serve` コマンドは、OpenHei クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `openhei.local` |
...
HTTP 基本認証でサーバーを保護するには、`OPENHEI_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `openhei` になるか、`OPENHEI_SERVER_USERNAME` を設定してオーバーライドします。これは、`openhei serve` と `openhei web` の両方に当てはまります。
...
`openhei` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
OpenHei サーバーを使用して、プログラムで OpenHei と対話します。
:::

`openhei serve` を実行してスタンドアロンサーバーを起動できます。すでに OpenHei TUI を実行している場合でも、`openhei serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、OpenHei [IDE](/docs/ide) プラグインによって使用されます。
