...
:::caution
モードは、OpenHei 設定の `agent` オプションを通じて設定されるようになりました。
...
これらを使用するか、OpenHei 設定を通じて独自の設定を行います。
...
OpenHei には 2 つの組み込みモードが付属しています。
...
- `edit` - 計画自体の詳細を示す `.openhei/plans/*.md` にあるファイルを除き、既存のファイルを変更できません
- `patch` - パッチを適用できません
...
- グローバル: `~/.config/openhei/modes/`
- プロジェクト: `.openhei/modes/`
...
温度が指定されていない場合、OpenHei はモデル固有のデフォルトを使用します (通常、ほとんどのモデルでは 0、Qwen モデルでは 0.55)。
...
モードファイルをプロジェクト固有モードの場合は `.openhei/modes/` に、グローバルモードの場合は `~/.config/openhei/modes/` に作成します。
