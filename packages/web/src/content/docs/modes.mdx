...
:::caution
Modes are now configured through the `agent` option in the openhei config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in openhei allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the openhei config.
...
openhei comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.openhei/plans/*.md` to detail the plan itself
...
Configure modes in your `openhei.json` config file:
...
both the global openhei config and the project specific config.
...
Create mode files in `.openhei/modes/` for project-specific modes or `~/.config/openhei/modes/` for global modes:
