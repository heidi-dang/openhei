...
مع OpenHei يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenHei سمتنا `openhei`.
...
يأتي OpenHei مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenHei مظهر terminal لديهم
...
يدعم OpenHei نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openhei/themes/*.json` أو `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openhei/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openhei/themes/*.json`
