---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenHei.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenHei ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenHei.
...
في `~/.local/share/openhei/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenHei.
...
OpenHei Zen هي قائمة نماذج يوفّرها فريق OpenHei وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenHei. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenHei Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر openhei، ثم انتقل إلى [openhei.ai/auth](https://openhei.ai/auth).
...
يعمل مثل أي مزوّد آخر في OpenHei واستخدامه اختياري بالكامل.
...
لاستخدام Amazon Bedrock مع OpenHei:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل openhei:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `openhei.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenHei غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل openhei بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات openhei.
...
تخزين الرمز في مساحة تخزين مصادقة openhei.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenHei نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenHei
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`openhei.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### التهيئة

خصّص عبر `openhei.json`:
...
لاستخدام اشتراك GitHub Copilot مع openhei:
...
لاستخدام Google Vertex AI مع OpenHei:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر openhei، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) لتسجيل كل محادثة OpenHei تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenHei مدرجة كجلسة مستقلة.
...
يمكنك ضبط openhei لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط openhei لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط openhei لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenHei. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/openhei) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenHei:
...
3. انسخ مفتاح API لاستخدامه في OpenHei.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenHei، يجب سحب معلومات النموذج محليا:
...
OpenHei Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenHei. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenHei Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenHei Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenHei.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات openhei.
...
5. يمكنك أيضا تخصيصها عبر إعدادات openhei. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات openhei. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع openhei. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `openhei.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenHei بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `openhei auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات openhei وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات openhei.
