...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في openhei.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في openhei مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.openhei/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/openhei/tools/`.
