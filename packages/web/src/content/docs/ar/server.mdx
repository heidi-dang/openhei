---
title: الخادم
description: تفاعل مع خادم openhei عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `openhei serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل openhei استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `openhei.local` |
...
عيّن `OPENHEI_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `openhei`، أو عيّن `OPENHEI_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `openhei serve` و `openhei web`.
...
عند تشغيل `openhei` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم openhei للتفاعل مع openhei برمجيا.
:::

تتيح هذه البنية لـ openhei دعم عدة عملاء وتمكّنك من التفاعل مع openhei برمجيا.

يمكنك تشغيل `openhei serve` لبدء خادم مستقل. إذا كانت واجهة openhei في terminal (TUI)
قيد التشغيل، فسيبدأ `openhei serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenHei لـ [IDE](/docs/ide).
...
يعرض خادم openhei واجهات API التالية.
