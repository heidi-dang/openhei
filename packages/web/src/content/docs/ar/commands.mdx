...
أنشئ الملف `.openhei/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenHei أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenHei:
...
- على مستوى النظام: `~/.config/openhei/commands/`
- لكل مشروع: `.openhei/commands/`
...
يتضمن openhei عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
