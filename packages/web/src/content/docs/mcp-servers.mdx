...
You can add external tools to OpenHei using the _Model Context Protocol_, or MCP. OpenHei supports both local and remote servers.
...
You can define MCP servers in your [OpenHei Config](https://openhei.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/openhei` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenHei automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenHei will:
...
If the server requires authentication, OpenHei will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `openhei mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenHei will store the tokens securely in `~/.local/share/openhei/mcp-auth.json`.
...
Your MCPs are available as tools in OpenHei, alongside built-in tools. So you can manage them through the OpenHei config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenHei to your Sentry account.
