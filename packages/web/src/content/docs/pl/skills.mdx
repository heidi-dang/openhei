...
Umiejętności agenta pozwalają openhei odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openhei przeszukuje te lokalizacje:

- Project config: `.openhei/skills/<name>/SKILL.md`
- Global config: `~/.config/openhei/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openhei przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openhei/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openhei/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openhei/skills/git-release/SKILL.md` w ten sposób:
...
openhei wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `openhei.json`:
...
**Dla agentów wbudowanych** (w `openhei.json`):
