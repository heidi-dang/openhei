...
Narzędzia stosowane do funkcji, z których LLM może korzystać podczas rozmów. Współpracują z [wbudowanymi narzędziami] (./tools) openhei, wtyczka jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.openhei/tools/` swojego projektu.
- Lub globalnie, umieszczając je w `~/.config/openhei/tools/`.
