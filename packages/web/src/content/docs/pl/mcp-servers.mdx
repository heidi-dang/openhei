...
Możesz dodać dodatkowe informacje do openhei za pomocą _Model Context Protocol_, w skrócie MCP. openhei obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [openhei Config](https://openhei.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/openhei`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
openhei automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openhei:
...
Jeśli serwer wymaga uwierzytelnienia, openhei poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openhei mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openhei bezpieczne przechowa tokeny w `~/.local/share/openhei/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openhei, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openhei, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openhei z kontem Sentry.
