...
Aby debugować problemy z openhei, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openhei\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `openhei --log-level DEBUG`.

---

## Przechowywanie danych

openhei przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/openhei/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\openhei`
...
openhei Desktop uruchamia w tle lokalny serwer openhei (sidecar `openhei-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `OpenHei` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/openhei/openhei.jsonc` (lub `~/.config/openhei/openhei.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/openhei/openhei.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openhei\openhei.jsonc`
...
openhei może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/openhei/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\openhei\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby openhei mógł ją odbudować.

1. Całkowicie zamknij openhei Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/openhei`
- **Linux**: usuń `~/.cache/openhei` (lub uruchom `rm -rf ~/.cache/openhei`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\openhei`

3. Uruchom ponownie openhei Desktop.

---

### Napraw problemy z połączeniem z serwerem

openhei Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `openhei.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `OPENHEI_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `OPENHEI_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows openhei Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami openhei.

---

### Brak powiadomień

openhei Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla openhei w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie openhei Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji openhei Desktop):

- `openhei.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `openhei.global.dat` i `openhei.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `openhei upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `openhei models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\openhei`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. openhei dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\openhei`

2. Uruchom ponownie kod openhei, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to openhei do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
openhei wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
