---
title: Serwer
description: Interakcja z serwerem openhei za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `openhei serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient openhei.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `openhei.local` |
...
Ustaw `OPENHEI_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `openhei` lub ustaw `OPENHEI_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `openhei serve`, jak i `openhei web`.
...
Kiedy uruchomisz `openhei`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera openhei do programowej interakcji z openhei.
:::

Ta architektura umożliwia obsługę wielu klientów przez openhei i programową interakcję z kodem otwartym.

Możesz uruchomić `openhei serve`, aby uruchomić samodzielny serwer. Jeśli masz
openhei TUI działa, `openhei serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki openhei [IDE](/docs/ide).
...
Serwer openhei udostępnia następujące interfejsy API.
