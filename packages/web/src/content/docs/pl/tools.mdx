...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OpenHei posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OpenHei.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENHEI_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENHEI_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenHei lub gdy zmienna środowiskowa `OPENHEI_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OpenHei:
