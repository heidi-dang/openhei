...
W OpenHei obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
OpenHei zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
OpenHei ma dwóch wbudowanych subagentów: **General** i **Explore**. Przyjrzymy się im poniżej.

---

## Wbudowane

OpenHei ma dwa wbudowane agenty główne i dwa wbudowane subagenty.
...
Skonfiguruj agenta w pliku `openhei.json`:
...
- Globalnie: `~/.config/openhei/agents/`
- W projekcie: `.openhei/agents/`
...
Jeśli nie zostanie skonfigurowana, OpenHei używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji OpenHei, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji OpenHei w `provider/model-id`. Na przykład, [OpenHei Zen](/docs/zen), użyj `openhei/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `openhei models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/heidi-dang/openhei).
:::
