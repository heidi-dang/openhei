...
:::caution
Tryby są teraz konfigurowane za pomocą opcji `agent` w konfiguracji openhei.
...
te lub skonfiguruj własne za pomocą konfiguracji openhei.
...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.openhei/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `openhei.json`:
...
- Globalnie: `~/.config/openhei/modes/`
...
Jeśli nie ma wpływu na temperaturę, openhei stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna openhei, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.openhei/modes/` dla trybów zapisanych dla projektu lub `~/.config/openhei/modes/` dla trybów globalnych:
