...
Dzięki openhei możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openhei używa naszego własnego motywu `openhei`.
...
openhei ma kilka wbudowanych motywów.
...
- Want openhei to match their terminal's appearance
...
openhei obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openhei/themes/*.json` lub `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openhei/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openhei/themes/*.json`
