---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w openhei.
---

import config from "../../../../config.mjs"
export const console = config.console

openhei używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji openhei.
...
w `~/.local/share/openhei/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim openhei
config.
...
OpenHei Zen to lista modeli dostarczonych przez zespół openhei, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z openhei. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenHei Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz openhei i przejdź do [openhei.ai/auth](https://openhei.ai/auth).
...
Działa jak każdy inny dostawca w openhei i jest całkowicie opcjonalny w użyciu.
...
Aby używać Amazon Bedrock z openhei:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania openhei:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `openhei.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w openhei nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod openhei działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji openhei.
...
openhei używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować openhei
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`openhei.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Konfiguracja

Customize through `openhei.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem openhei:
...
Aby używać Google Vertex AI z openhei:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Set them while running openhei.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą openhei, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session), aby automatycznie rejestrować każdą konwersację openhei jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja openhei będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować openhei tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować openhei tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować openhei tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem openhei. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/openhei).
:::
...
Aby korzystać z Ollama Cloud z openhei:
...
3. Skopiuj klucz API do użycia w openhei.
...
6. **Ważne**: Przed użyciem modeli chmurowych w openhei musisz pobrać informacje o modelu lokalnie:
...
OpenHei Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół openhei. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenHei Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenHei Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API openhei.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji openhei.
...
5. Możesz je także dostosować za pomocą konfiguracji openhei. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z openhei:
...
Możesz także dostosować modele za pomocą konfiguracji openhei. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z openhei. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `openhei.json` w katalogu projektu:
...
Pola `limit` pozwalają openhei zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `openhei auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację openhei i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji openhei.
