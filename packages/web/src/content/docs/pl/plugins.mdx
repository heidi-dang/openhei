---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openhei.
---

Wtyczki umożliwiające wyłączenie openhei poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openhei.
...
- `.openhei/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openhei/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openhei/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openhei/openhei.json`)
2. Konfiguracja projektu (`openhei.json`)
3. Globalny katalog wtyczek (`~/.config/openhei/plugins/`)
4. Katalog wtyczek projektu (`.openhei/plugins/`)
...
openhei uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openhei do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openhei.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openhei, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openhei czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openhei:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openhei. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://openhei.ai/docs/sdk).
