...
Create `.openhei/commands/test.md`:
...
You can add custom commands through the OpenHei config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenHei [config](/docs/config):
...
openhei includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
