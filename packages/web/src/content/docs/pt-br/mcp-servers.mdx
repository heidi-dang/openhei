...
Você pode adicionar ferramentas externas ao openhei usando o _Model Context Protocol_, ou MCP. O openhei suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do openhei](https://openhei.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/openhei`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O openhei lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o openhei irá:
...
Se o servidor requer autenticação, o openhei solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `openhei mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o openhei armazenará os tokens de forma segura em `~/.local/share/openhei/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no openhei, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do openhei como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o openhei à sua conta do Sentry.
