---
title: Plugins
description: Escreva seus próprios plugins para estender o openhei.
---

Plugins permitem que você estenda o openhei conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openhei.
...
- `.openhei/plugins/` - Plugins em nível de projeto
- `~/.config/openhei/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openhei/node_modules/`.
...
1. Configuração global (`~/.config/openhei/openhei.json`)
2. Configuração do projeto (`openhei.json`)
3. Diretório de plugins global (`~/.config/openhei/plugins/`)
4. Diretório de plugins do projeto (`.openhei/plugins/`)
...
O openhei executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do openhei para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openhei.
...
:::note
Se você estiver usando o aplicativo desktop openhei, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openhei de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao openhei:
...
O helper `tool` cria uma ferramenta personalizada que o openhei pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openhei junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://openhei.ai/docs/sdk) para detalhes.
