---
title: Provedores
description: Usando qualquer provedor LLM no openhei.
---

import config from "../../../../config.mjs"
export const console = config.console

openhei usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do openhei.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/openhei/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do openhei.
...
OpenHei Zen é uma lista de modelos fornecidos pela equipe do openhei que foram testados e verificados para funcionar bem com o openhei. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o OpenHei Zen.
:::

1. Execute o comando `/connect` no TUI, selecione openhei e acesse [openhei.ai/auth](https://openhei.ai/auth).
...
Funciona como qualquer outro provedor no openhei e é completamente opcional.
...
Para usar o Amazon Bedrock com o openhei:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   Defina uma dessas variáveis de ambiente ao executar o openhei:
   ...
   Para configuração específica do projeto ou persistente, use `openhei.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no openhei não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o openhei funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do openhei.
...
armazenar o token no armazenamento de autenticação do openhei.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
openhei usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o openhei
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `openhei.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth).

##### Configuração

Personalize através do `openhei.json`:
...
Para usar sua assinatura do GitHub Copilot com o openhei:
...
Para usar o Google Vertex AI com o openhei:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o openhei.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do openhei, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) para registrar automaticamente cada conversa do openhei como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do openhei listada como uma sessão separada.
...
Você pode configurar o openhei para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o openhei para usar modelos locais através do LM Studio.
...
Você pode configurar o openhei para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o openhei. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/openhei) para detalhes.
:::
...
Para usar o Ollama Cloud com o openhei:
...
3. Copie a chave da API para uso no openhei.
...
6. **Importante**: Antes de usar modelos em nuvem no openhei, você deve puxar as informações do modelo localmente:
...
OpenHei Zen é uma lista de modelos testados e verificados fornecidos pela equipe do openhei. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>OpenHei Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **OpenHei Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do openhei.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do openhei.
...
5. Você também pode personalizá-los através da sua configuração do openhei. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o openhei:
...
Você também pode personalizar modelos através da sua configuração do openhei. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o openhei. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
1. Execute o comando `/connect` e role para baixo até **Outro**.
   ...
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no openhei.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `openhei.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o openhei entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `openhei auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do openhei e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do openhei.
