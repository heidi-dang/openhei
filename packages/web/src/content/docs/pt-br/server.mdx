---
title: Servidor
description: Interaja com o servidor openhei via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `openhei serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente openhei pode usar.
...
```bash
openhei serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `openhei.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
openhei serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENHEI_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `openhei`, ou defina `OPENHEI_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `openhei serve` quanto ao `openhei web`.

```bash
OPENHEI_SERVER_PASSWORD=your-password openhei serve
```
...
Quando você executa `openhei`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor openhei para interagir com o openhei programaticamente.
:::

Essa arquitetura permite que o openhei suporte múltiplos clientes e permite que você interaja com o openhei programaticamente.

Você pode executar `openhei serve` para iniciar um servidor autônomo. Se você tiver o TUI do openhei em execução, `openhei serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do openhei [IDE](/docs/ide).
...
O servidor openhei expõe as seguintes APIs.
