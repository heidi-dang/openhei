...
As habilidades do agente permitem que o openhei descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openhei pesquisa nesses locais:

- Configuração do projeto: `.openhei/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openhei/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openhei sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openhei/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openhei/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openhei/skills/git-release/SKILL.md` assim:
...
O openhei lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `openhei.json`:
...
**Para agentes embutidos** (em `openhei.json`):
