...
Com o openhei, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openhei usa nosso próprio tema `openhei`.
...
O openhei vem com vários temas embutidos.
...
- Querem que o openhei corresponda à aparência do seu terminal
...
O openhei suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openhei/themes/*.json` ou `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openhei/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openhei/themes/*.json`
