---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenHei.
---

Plugins giver dig mulighed for at udvide OpenHei ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenHeis standardadfærd.
...
- `.openhei/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openhei/node_modules/`.
...
1. Global konfiguration (`~/.config/openhei/openhei.json`)
2. Projektkonfiguration (`openhei.json`)
3. Global plugin-mappe (`~/.config/openhei/plugins/`)
4. Projekt plugin bibliotek (`.openhei/plugins/`)
...
OpenHei kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En openhei SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openhei.
...
:::note
Hvis du bruger OpenHei desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå openhei i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openhei:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openhei kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openhei sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://openhei.ai/docs/sdk) for detaljer.
