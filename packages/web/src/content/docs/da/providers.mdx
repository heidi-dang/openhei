---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i OpenHei.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHei bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i OpenHei-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/openhei/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i OpenHei-konfigurationen.
...
OpenHei Zen er en liste over modeller leveret af OpenHei-teamet, som er blevet testet og verificeret til at fungere godt med OpenHei. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med OpenHei Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg openhei og gå til [openhei.ai/auth](https://openhei.ai/auth).
...
Det fungerer som alle andre udbydere i OpenHei og er helt valgfrit at bruge.
...
Sådan bruger du Amazon Bedrock med OpenHei:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører openhei:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `openhei.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i OpenHei understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at openhei skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem openhei-konfigurationen.
...
gemme tokenet i openhei auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
OpenHei bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse OpenHei
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`openhei.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `openhei.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med OpenHei:
...
Sådan bruger du Google Vertex AI med OpenHei:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem openhei, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) plugin for automatisk at logge hver OpenHei-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver OpenHei-samtale opført som en separat session.
...
Du kan konfigurere openhei til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere openhei til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere openhei til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenHei. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/openhei) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med OpenHei:
...
3. Kopier API-nøglen til brug i OpenHei.
...
6. **Vigtigt**: Før du bruger skymodeller i OpenHei, skal du hente modelinformationen lokalt:
...
OpenHei Zen er en liste over testede og verificerede modeller leveret af OpenHei-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>OpenHei Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **OpenHei Zen**.

   ```txt
   /connect
   ```

3. Indtast OpenHei API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem openhei-konfigurationen.
...
5. Du kan også tilpasse dem gennem openhei-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenHei:
...
Du kan også tilpasse modeller gennem openhei-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med openhei. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `openhei.json`-filen i projektkataloget:
...
`limit`-felterne lader OpenHei forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `openhei auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek openhei-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i openhei-konfigurationen.
