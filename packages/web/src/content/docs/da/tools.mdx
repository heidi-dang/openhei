...
Her er alle de innebygde verktøyene som er tilgængelige i OpenHei.
...
:::note
Dette verktøyet er bare tilgængelig når `OPENHEI_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENHEI_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenHei-leverandøren eller når miljøvariabelen `OPENHEI_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
