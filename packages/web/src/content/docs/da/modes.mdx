...
:::caution
Tilstande er nu konfigureret gennem indstillingen `agent` i openhei-konfigurationen. De
`mode` mulighed er nu forældet. [Learn more](/docs/agents).
:::

Tilstande i openhei giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.

Den kommer med to indbyggede tilstande: **build** og **plan**. Du kan tilpasse
disse eller konfigurer dine egne gennem openhei-konfigurationen.
...
openhei leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.openhei/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `openhei.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger openhei modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
både den globale openhei-konfiguration og den projektspecifikke konfiguration.
...
Opret tilstandsfiler i `.openhei/modes/` for projektspecifikke tilstande eller `~/.config/openhei/modes/` for globale tilstande:
