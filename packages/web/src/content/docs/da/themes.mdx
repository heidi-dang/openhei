...
Med OpenHei kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenHei vårt eget `openhei`-tema.
...
OpenHei støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openhei/themes/*.json` eller `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openhei/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openhei/themes/*.json`
