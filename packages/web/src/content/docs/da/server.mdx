---
title: Server
description: Interager med openhei-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openhei serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en openhei-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `openhei.local` |
...
Indstil `OPENHEI_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openhei`, eller indstil `OPENHEI_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openhei serve` og `openhei web`.
...
Når du kører `openhei` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug openhei-serveren til at interagere med openhei programmatisk.
:::

Denne arkitektur lader openhei understøtte flere klienter og lader dig interagere med openhei programmatisk.

Du kan køre `openhei serve` for at starte en selvstændig server. Hvis du har
openhei TUI kørende, vil `openhei serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenHei [IDE](/docs/ide) plugins.
...
OpenHei-serveren viser følgende API'er.
