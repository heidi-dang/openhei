...
For at fejlfinde problemer med OpenHei, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openhei\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `openhei --log-level DEBUG`.

---

## Lagring

openhei gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/openhei/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\openhei`
...
OpenHei Desktop kører en lokal OpenHei-server (`openhei-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `OpenHei`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/openhei/openhei.jsonc` (eller `~/.config/openhei/openhei.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/openhei/openhei.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openhei\openhei.jsonc`
...
OpenHei kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/openhei/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\openhei\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så OpenHei kan genopbygge den.

1. Afslut OpenHei Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/openhei`
- **Linux**: slet `~/.cache/openhei` (eller kør `rm -rf ~/.cache/openhei`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\openhei`

3. Start OpenHei Desktop på ny.

---

### Løs problemer med serverforbindelse

OpenHei Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `openhei.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `OPENHEI_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OPENHEI_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver OpenHei Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i OpenHei.

---

### Meddelelser vises ikke

OpenHei Desktop viser kun systemvarsler når:

- varsler er aktiveret for OpenHei i dine OS-indstillinger, og
...
1. Afslut OpenHei Desktop.
2. Find og slet disse filer (de findes i OpenHei Desktop-appens datamappe):

- `openhei.settings.dat` (skrivebordsstandardserver URL)
- `openhei.global.dat` og `openhei.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med OpenHei:
...
For at finde ud af hvilke modeller du har adgang til, kør `openhei models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\openhei`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. openhei installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\openhei`

2. Start openhei på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge openhei til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
openhei vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
