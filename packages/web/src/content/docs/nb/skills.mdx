...
Agentferdigheter lar OpenHei oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
openhei søker etter disse stedene:

- Prosjektkonfigurasjon: `.openhei/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/openhei/skills/<name>/SKILL.md`
...
For prosjektlokale stier går openhei opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.openhei/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/openhei/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.openhei/skills/git-release/SKILL.md` slik:
...
openhei viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `openhei.json`:
...
**For innebygde agenter** (i `openhei.json`):
