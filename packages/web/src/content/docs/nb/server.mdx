---
title: Tjener
description: Samhandle med OpenHei-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openhei serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenHei-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `openhei.local` |
...
Sett `OPENHEI_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `openhei` som standard, eller sett `OPENHEI_SERVER_USERNAME` for å overstyre det. Dette gjelder både `openhei serve` og `openhei web`.
...
Når du kjører `openhei` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenHei-serveren til å samhandle med OpenHei programmatisk.
:::

Denne arkitekturen lar OpenHei støtte flere klienter og lar deg samhandle med OpenHei programmatisk.

Du kan kjøre `openhei serve` for å starte en frittstående server. Hvis du har
OpenHei TUI kjører, vil `openhei serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenHei [IDE](/docs/ide) plugins.
...
openhei-serveren viser følgende APIer.
