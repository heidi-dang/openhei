...
description: Skriv dine egne programtillegg for å utvide openhei.
---

Plugins lar deg utvide OpenHei ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenHei.
...
- `.openhei/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/openhei/node_modules/`.
...
1. Global konfigurasjon (`~/.config/openhei/openhei.json`)
2. Prosjektkonfigurasjon (`openhei.json`)
3. Global plugin-katalog (`~/.config/openhei/plugins/`)
4. Prosjektpluginkatalog (`.openhei/plugins/`)
...
OpenHei kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenHei SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenHei.
...
:::note
Hvis du bruker OpenHei-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenHei fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenHei:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenHei kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenHei sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://openhei.ai/docs/sdk) for detaljer.
