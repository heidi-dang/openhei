...
Du kan legge til eksterne verktøy i OpenHei ved å bruke _Model Context Protocol_ eller MCP. OpenHei støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenHei Config](https://openhei.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="openhei.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/openhei`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
OpenHei håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenHei:
...
Hvis serveren krever autentisering, vil OpenHei be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `openhei mcp auth <server-name>`.
...
```bash
openhei mcp auth my-oauth-server
```
...
```bash
openhei mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenHei lagre tokens sikkert i `~/.local/share/openhei/mcp-auth.json`.
...
openhei mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenHei, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenHei-konfigurasjonen som alle andre verktøy.
...
```json title="openhei.json" {11, 14-18}
{
  "$schema": "https://openhei.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
openhei mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenHei til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenHei-samtale oppført som en separat økt.
