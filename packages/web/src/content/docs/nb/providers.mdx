---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenHei.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenHei bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenHei-konfigurasjonen.
...
i `~/.local/share/openhei/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenHei
konfig.
...
OpenHei Zen er en liste over modeller levert av OpenHei-teamet som har vært
testet og verifisert for å fungere godt med OpenHei. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenHei Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg openhei og gå til [openhei.ai/auth](https://openhei.ai/auth).
...
Det fungerer som alle andre leverandører i OpenHei og er helt valgfritt å bruke.
...
Slik bruker du Amazon Bedrock med OpenHei:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenHei:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `openhei.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i openhei støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenHei skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenHei-konfigurasjonen.
...
for å lagre token i OpenHei auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenHei bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenHei
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`openhei.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [openhei-gitlab-auth](https://www.npmjs.com/package/@gitlab/opencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `openhei.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OpenHei:
...
Slik bruker du Google Vertex AI med OpenHei:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenHei.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenHei, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [openhei-helicone-session](https://github.com/H2Shami/openhei-helicone-session) plugin for automatisk å logge hver OpenHei-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenHei-samtale oppført som en separat økt.
...
Du kan konfigurere OpenHei for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenHei for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenHei for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenHei. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/openhei) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenHei:
...
3. Kopier API-nøkkelen for bruk i OpenHei.
...
6. **Viktig**: Før du bruker skymodeller i OpenHei, må du hente modellinformasjonen lokalt:
...
OpenHei Zen er en liste over testede og verifiserte modeller levert av OpenHei-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenHei Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenHei Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenHei API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenHei-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenHei-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenHei:
...
Du kan også tilpasse modeller gjennom OpenHei-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenHei. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `openhei.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenHei forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `openhei auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenHei-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenHei-konfigurasjonen.
