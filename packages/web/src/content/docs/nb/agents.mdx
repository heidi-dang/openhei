...
Det er to typer agenter i OpenHei; primære agenter og underagenter.
...
OpenHei kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenHei kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenHei kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `openhei.json`:
...
- Globalt: `~/.config/openhei/agents/`
- Per prosjekt: `.openhei/agents/`
...
Hvis ingen temperatur er spesifisert, bruker OpenHei modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenHei-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenHei-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenHei Zen](/docs/zen), vil du bruke `openhei/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `openhei models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/heidi-dang/openhei).
:::
