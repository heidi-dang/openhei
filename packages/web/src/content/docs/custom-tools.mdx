---
title: Custom Tools
description: Create tools the LLM can call in openhei.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside openhei's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.openhei/tools/` directory of your project.
- Or globally, by placing them in `~/.config/openhei/tools/`.
...
```ts title=".openhei/tools/bash.ts"
import { tool } from "@openhei-ai/plugin"
