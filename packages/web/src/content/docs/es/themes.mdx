...
Con OpenHei puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OpenHei usa nuestro propio tema `openhei`.
...
OpenHei viene con varios temas integrados.
...
- Quiere que OpenHei coincida con la apariencia de su terminal
...
OpenHei admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/openhei/themes/*.json` o `$XDG_CONFIG_HOME/openhei/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.openhei/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.openhei/themes/*.json`
