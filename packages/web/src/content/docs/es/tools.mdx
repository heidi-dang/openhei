...
Las herramientas permiten que LLM realice acciones en su código base. OpenHei viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OpenHei.
...
:::note
Esta herramienta solo está disponible cuando `OPENHEI_EXPERIMENTAL_LSP_TOOL=true` (o `OPENHEI_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenHei o cuando la variable de entorno `OPENHEI_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenHei:

```bash
OPENHEI_ENABLE_EXA=1 openhei
```
