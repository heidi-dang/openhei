---
title: Complementos
description: Escriba sus propios complementos para extender OpenHei.
---

Los complementos le permiten extender OpenHei al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenHei.
...
- `.openhei/plugins/` - Complementos a nivel de proyecto
- `~/.config/openhei/plugins/` - Complementos globales
...
```json title="openhei.json"
{
  ...
  "plugin": ["openhei-helicone-session", "openhei-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openhei/node_modules/`.
...
1. Configuración global (`~/.config/openhei/openhei.json`)
2. Configuración del proyecto (`openhei.json`)
3. Directorio global de complementos (`~/.config/openhei/plugins/`)
4. Directorio de complementos del proyecto (`.openhei/plugins/`)
...
```json title=".openhei/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenHei ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".openhei/plugins/my-plugin.ts"
...
```js title=".openhei/plugins/example.js"
...
- `client`: Un cliente SDK openhei para interactuar con la IA.
...
import type { Plugin } from "@openhei-ai/plugin"
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openhei.
...
```js title=".openhei/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openhei"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenHei, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que openhei lea archivos `.env`:

```javascript title=".openhei/plugins/env-protection.js"
...
```javascript title=".openhei/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a openhei:

```ts title=".openhei/plugins/custom-tools.ts"
import { type Plugin, tool } from "@openhei-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que openhei puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openhei junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://openhei.ai/docs/sdk) para obtener más detalles.
...
```ts title=".openhei/plugins/compaction.ts"
import type { Plugin } from "@openhei-ai/plugin"
...
```ts title=".openhei/plugins/custom-compaction.ts"
import type { Plugin } from "@openhei-ai/plugin"
