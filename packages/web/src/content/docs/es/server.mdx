---
title: Servidor
description: Interactuar con el servidor openhei a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openhei serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openhei puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `openhei.local` |
...
Configure `OPENHEI_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openhei`, o configure `OPENHEI_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openhei serve` como a `openhei web`.
...
Cuando ejecuta `openhei`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openhei para interactuar con openhei mediante programación.
:::

Esta arquitectura permite que openhei admita múltiples clientes y le permite interactuar con openhei mediante programación.

Puede ejecutar `openhei serve` para iniciar un servidor independiente. Si tienes el
openhei TUI ejecutándose, `openhei serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenHei [IDE](/docs/ide).
...
El servidor openhei expone las siguientes API.
