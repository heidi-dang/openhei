...
Hay dos tipos de agentes en OpenHei; agentes primarios y subagentes.
...
OpenHei viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
OpenHei viene con dos subagentes integrados, **General** y **Explore**. Veremos esto a continuación.

---

## Integrados

OpenHei viene con dos agentes primarios integrados y dos subagentes integrados.
...
Configure agentes en su archivo de configuración `openhei.json`:
...
Si no se especifica ninguna temperatura, OpenHei utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global OpenHei como para la configuración específica del proyecto.
...
El ID del modelo en su configuración OpenHei usa el formato `provider/model-id`. Por ejemplo, si usa [OpenHei Zen](/docs/zen), usaría `openhei/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `openhei models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/heidi-dang/openhei).
:::
