...
Las habilidades del agente permiten a OpenHei descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenHei busca estas ubicaciones:

- Configuración del proyecto: `.openhei/skills/<name>/SKILL.md`
- Configuración global: `~/.config/openhei/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenHei sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.openhei/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/openhei/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.openhei/skills/git-release/SKILL.md` así:
...
OpenHei enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `openhei.json`:
...
**Para agentes integrados** (en `openhei.json`):
