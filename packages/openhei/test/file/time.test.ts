...
describe("file/time", () => {
  ...
  describe("assert()", () => {
    ...
    test("skips check when OPENHEI_DISABLE_FILETIME_CHECK is true", async () => {
      await using tmp = await tmpdir()
      const filepath = path.join(tmp.path, "file.txt")
      await fs.writeFile(filepath, "content", "utf-8")
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const original = Flag.OPENHEI_DISABLE_FILETIME_CHECK
          ;(Flag as { OPENHEI_DISABLE_FILETIME_CHECK: boolean }).OPENHEI_DISABLE_FILETIME_CHECK = true

          try {
            // Should not throw even though file wasn't read
            await FileTime.assert(sessionID, filepath)
          } finally {
            ;(Flag as { OPENHEI_DISABLE_FILETIME_CHECK: boolean }).OPENHEI_DISABLE_FILETIME_CHECK = original
          }
        },
      })
    })
