...
const managedConfigDir = process.env.OPENHEI_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "openhei.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "openhei.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "openhei.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "openhei.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .openhei directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openheiDir = path.join(dir, ".openhei")
      await fs.mkdir(openheiDir, { recursive: true })
      const agentDir = path.join(openheiDir, "agent")
...
test("loads agents from .openhei/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openheiDir = path.join(dir, ".openhei")
      await fs.mkdir(openheiDir, { recursive: true })

      const agentsDir = path.join(openheiDir, "agents")
...
test("loads commands from .openhei/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openheiDir = path.join(dir, ".openhei")
      await fs.mkdir(openheiDir, { recursive: true })

      const commandDir = path.join(openheiDir, "command")
...
test("loads commands from .openhei/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const openheiDir = path.join(dir, ".openhei")
      await fs.mkdir(openheiDir, { recursive: true })

      const commandsDir = path.join(openheiDir, "commands")
...
test("does not try to install dependencies in read-only OPENHEI_CONFIG_DIR", async () => {
  ...
  const prev = process.env.OPENHEI_CONFIG_DIR
  process.env.OPENHEI_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.OPENHEI_CONFIG_DIR
    else process.env.OPENHEI_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable OPENHEI_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "openhei.json"),
        JSON.stringify({ $schema: "https://openhei.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "openhei.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .openhei config
      const projectDir = path.join(dir, "project")
      const openheiDir = path.join(projectDir, ".openhei")
      await fs.mkdir(openheiDir, { recursive: true })
      ...
      // Local .openhei config with different plugins
      await Filesystem.write(
        path.join(openheiDir, "openhei.json"),
        JSON.stringify({
          $schema: "https://openhei.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openhei config with some overlapping plugins
      await Filesystem.write(
        path.join(openheiDir, "openhei.json"),
        JSON.stringify({
          $schema: "https://openhei.ai/config.json",
...
// Note: preload.ts sets OPENHEI_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .openhei config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .openhei directory config enables it
      const openheiDir = path.join(dir, ".openhei")
      await fs.mkdir(openheiDir, { recursive: true })
      await Filesystem.write(
        path.join(openheiDir, "openhei.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/openhei")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/openhei")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-openhei@2.4.3")).toBe("oh-my-openhei")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@openhei/plugin@2.0.0")).toBe("@openhei/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-openhei@2.4.3", "file:///project/.openhei/plugin/oh-my-openhei.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.openhei/plugin/oh-my-openhei.js")
  })
  ...
  test("local plugin directory overrides global openhei.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const openheiDir = path.join(projectDir, ".openhei")
        const pluginDir = path.join(openheiDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("OPENHEI_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["OPENHEI_DISABLE_PROJECT_CONFIG"]
    process.env["OPENHEI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["OPENHEI_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["OPENHEI_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .openhei/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENHEI_DISABLE_PROJECT_CONFIG"]
    process.env["OPENHEI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .openhei directory with a command
          const openheiDir = path.join(dir, ".openhei", "command")
          await fs.mkdir(openheiDir, { recursive: true })
          await Filesystem.write(path.join(openheiDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .openhei should NOT be in directories list
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["OPENHEI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENHEI_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["OPENHEI_CONFIG_DIR"]
      process.env["OPENHEI_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["OPENHEI_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["OPENHEI_CONFIG_DIR"]
      } else {
        process.env["OPENHEI_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("OPENHEI_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["OPENHEI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["OPENHEI_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["OPENHEI_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["OPENHEI_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from OPENHEI_CONFIG_DIR, not project
...
describe("OPENHEI_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in OPENHEI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENHEI_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["OPENHEI_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://openhei.ai/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["OPENHEI_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["OPENHEI_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in OPENHEI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["OPENHEI_CONFIG_CONTENT"]
