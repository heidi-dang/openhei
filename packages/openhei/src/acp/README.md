...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for openhei.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal openhei sessions
...
openhei acp --cwd /path/to/project
```
...
```bash
OPENHEI_ENABLE_QUESTION_TOOL=1 openhei acp
```
...
- **Terminal Integration**: Full terminal support via openhei's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | openhei acp
```
...
ACP sessions map cleanly to openhei's internal session model:
