...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse } from "@openhei-ai/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `openhei auth login` in the terminal",
        name: "Login with openhei",
        id: "openhei-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // OpenHei stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const openheiProvider = providers.find((p) => p.id === "openhei")
    if (openheiProvider) {
      if (openheiProvider.models["big-pickle"]) {
        return { providerID: "openhei", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(openheiProvider.models))
      if (best) {
        return {
