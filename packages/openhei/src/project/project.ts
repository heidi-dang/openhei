...
import { fn } from "@openhei-ai/util/fn"
  ...
  export async function fromDirectory(directory: string) {
    ...
    const data = await iife(async () => {
      ...
      if (dotgit) {
        ...
        let id = await Filesystem.readText(path.join(dotgit, "openhei"))
          .then((x) => x.trim())
          .catch(() => undefined)
        ...
        if (!id) {
          ...
          id = roots[0]
          if (id) {
            void Filesystem.write(path.join(dotgit, "openhei"), id).catch(() => undefined)
          }
        }
    ...
    if (Flag.OPENHEI_EXPERIMENTAL_ICON_DISCOVERY) discover(existing)
