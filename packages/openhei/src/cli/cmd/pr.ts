...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run openhei",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for openhei session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found openhei session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`openhei import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting openhei...")
        UI.println()

        // Launch openhei TUI with session ID if available
        const { spawn } = await import("child_process")
        const openheiArgs = sessionId ? ["-s", sessionId] : []
        const openheiProcess = spawn("openhei", openheiArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          openheiProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`openhei exited with code ${code}`))
          })
          openheiProcess.on("error", reject)
        })
      },
    })
  },
