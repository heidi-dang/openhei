...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall openhei and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenHei")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g openhei-ai",
      pnpm: "pnpm uninstall -g openhei-ai",
      bun: "bun remove -g openhei-ai",
      yarn: "yarn global remove openhei-ai",
      brew: "brew uninstall openhei",
      choco: "choco uninstall openhei",
      scoop: "scoop uninstall openhei",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall openhei -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenHei!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# openhei") || content.includes(".openhei/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".openhei/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".openhei/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".openhei"))
    ) {
      continue
    }
