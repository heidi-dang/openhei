...
export class JobRunner {
  ...
  private async log(jobId: string, level: 'info' | 'warn' | 'error' | 'success', message: string, phase?: JobPhase): Promise<void> {
    await this.jobManager.appendLog(jobId, {
      timestamp: Date.now(),
      level,
      message,
  ...
  async stopJob(jobId: string): Promise<{
    ...
    if (job.backend?.status === 'running' || job.backend?.status === 'error') {
      ...
      if (stopResult.verified && stopResult.portClosed) {
        ...
        await this.log(jobId, 'warn', `Backend stop incomplete (verified: ${stopResult.verified}, port closed: ${stopResult.portClosed})`)
      }
    }
    ...
    if (job.frontend?.status === 'running' || job.frontend?.status === 'error') {
      ...
      if (stopResult.verified && stopResult.portClosed) {
        ...
        await this.log(jobId, 'warn', `Frontend stop incomplete (verified: ${stopResult.verified}, port closed: ${stopResult.portClosed})`)
      }
    }
