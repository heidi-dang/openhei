...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openhei://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openhei://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openhei://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openhei://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openhei://open-project")).toBeUndefined()
    expect(parseDeepLink("openhei://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openhei://open-project?directory=/a",
      "openhei://other?directory=/b",
      "openhei://open-project?directory=/c",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENHEI__: {
        deepLinks: ["openhei://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENHEI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openhei://open-project?directory=/a"])
