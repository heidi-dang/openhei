...
export const popularProviders = ["openhei", "anthropic", "github-copilot", "openai", "google", "openrouter", "vercel"]
...
export function useProviders() {
  ...
  const paid = createMemo(() =>
    connected().filter((p) => p.id !== "openhei" || Object.values(p.models).find((m) => m.cost?.input)),
  )
