...
For "ui" mode, the data should follow this structure:
{
  ...
  onMount(() => {
    setMessages([{ role: "assistant", content: "Hi! I'm Gemini, your App Builder assistant. What kind of app would you like to build today?" }])
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const contextMessages = messages().map(m => `[${m.role.toUpperCase()}]: ${m.content}`).join("\n\n")
      ...
      while (!found && attempts < 60 && isMounted) {
        ...
        if (lastMsg && (lastMsg as any).id > lastUserMsgId) {
          ...
          if (content || toolCalls.every((tool: any) => (tool as any).state?.status === "completed" || (tool as any).state?.status === "error")) {
            setMessages((prev) => {
    ...
    <>
      ...
      <div class="flex flex-col h-[750px] lg:h-[85vh] border border-white/20 rounded-[2rem] overflow-hidden bg-white/40 backdrop-blur-2xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)] transition-all duration-700 ease-in-out">
        ...
        <div class="flex flex-1 overflow-hidden bg-gradient-to-br from-[#f8f9ff] to-[#f0f4ff] relative">
          ...
          <aside
            class={`absolute inset-y-0 left-0 w-[300px] lg:relative lg:translate-x-0 transition-transform duration-300 ease-out z-10 border-r border-white/40 bg-white/70 backdrop-blur-xl p-6 flex flex-col gap-5 shrink-0 shadow-2xl lg:shadow-none ${showLeftPanel() ? "translate-x-0" : "-translate-x-full"
              }`}
          >
          ...
          <main class="flex-1 flex flex-col min-w-0 bg-transparent relative z-0">
            <ScrollView class="flex-1 p-4 lg:p-8" ref={scrollRef}>
              <div class="max-w-3xl mx-auto space-y-10 py-4">
                <For each={messages()}>
                  {(msg) => (
                    <div class={`flex gap-4 group transition-all duration-500 ease-out animate-in slide-in-from-bottom-4 fill-mode-both ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                      <div class={`w-10 h-10 rounded-2xl shrink-0 flex items-center justify-center shadow-lg transition-transform group-hover:scale-110 ${msg.role === "user" ? "bg-gradient-to-br from-amber-500 to-orange-600 shadow-amber-500/20" : "bg-gradient-to-br from-blue-600 to-indigo-700 shadow-blue-500/20"
                        }`}>
                        <Icon
                          name={msg.role === "user" ? "prompt" : "brain"}
                      ...
                      <div class={`flex-1 min-w-0 max-w-[85%] ${msg.role === "user" ? "text-right" : ""}`}>
                        ...
                        <div class={`p-5 rounded-2xl text-[15px] leading-relaxed shadow-sm border transition-colors ${msg.role === "user"
                          ...
                          <Show when={msg.tools && msg.tools.length > 0}>
                            <div class="mt-4 pt-4 border-t border-slate-100 flex flex-wrap gap-2">
                              <For each={msg.tools}>
                                {(tool: any) => (
                                  <div class={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 border shadow-sm transition-all ${tool.state?.status === "completed" ? "bg-green-50 border-green-100 text-green-700" :
                                    tool.state?.status === "error" ? "bg-red-50 border-red-100 text-red-700" :
                                      "bg-blue-50 border-blue-100 text-blue-700 animate-pulse"
                                    }`}>
                                    <Icon name={tool.state?.status === "completed" ? "brain" : "dash"} class="w-3 h-3" />
                                    <span>{tool.tool}</span>
                                  </div>
                                )}
                              </For>
                  ...
                  <div class="flex gap-4 animate-in fade-in slide-in-from-left-4 duration-300">
                    ...
                    <div class="flex-1 space-y-3">
                      ...
                      <div class="inline-flex items-center gap-3 px-5 py-3 bg-white/60 backdrop-blur-sm border border-white rounded-2xl shadow-sm">
                        <Show when={agentActivity()} fallback={
          ...
          <aside
            class={`absolute inset-y-0 right-0 w-[300px] lg:relative lg:translate-x-0 transition-transform duration-300 ease-out z-10 border-l border-white/40 bg-white/70 backdrop-blur-xl p-6 flex flex-col gap-8 shrink-0 overflow-y-auto shadow-2xl lg:shadow-none ${showRightPanel() ? "translate-x-0" : "translate-x-full"
              }`}
          >
            ...
            <div>
              <h3 class="text-[11px] font-black uppercase tracking-[0.2em] text-blue-600/60 mb-5">Model Intelligence</h3>
              <div class="relative">
                <select
                ...
                >
                  <For each={models.list().filter(m => m.id.includes("gemini"))}>
                    {(model) => (
                      <option value={model.id}>{model.id.split("/").pop()}</option>
                    )}
                  </For>
            ...
            <div class="space-y-6">
              ...
              <div class="space-y-3">
                <div class="flex justify-between items-center px-0.5">
                  ...
                  <span class="text-[12px] font-mono text-blue-600 bg-blue-50 px-2 py-0.5 rounded leading-none">{temperature()}</span>
                </div>
                <input
                  type="range"
                  min="0"
              ...
              <div class="space-y-3">
                <div class="flex justify-between items-center px-0.5">
                  ...
                  <span class="text-[12px] font-mono text-blue-600 bg-blue-50 px-2 py-0.5 rounded leading-none">{topK()}</span>
                </div>
                <input
                  type="range"
                  min="1"
              ...
              <div class="space-y-3">
                <div class="flex justify-between items-center px-0.5">
                  ...
                  <span class="text-[12px] font-mono text-blue-600 bg-blue-50 px-2 py-0.5 rounded leading-none">{topP()}</span>
                </div>
                <input
                  type="range"
                  min="0"
            ...
            <div class="mt-auto pt-6 border-t border-surface-subtle">
              <div class="flex items-center gap-2 text-text-weak group cursor-help">
                ...
                <span class="text-[11px] group-hover:text-text-base transition-colors">Advanced configurations are applied to your current build session.</span>
              </div>
            </div>
          </aside>
        </div>
