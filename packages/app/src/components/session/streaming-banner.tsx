...
export type BannerType = "replaying" | "resync_required" | "retry" | "resync" | undefined
...
export function StreamingBanner(props: StreamingBannerProps) {
  ...
  const bannerMessage = () => {
    switch (props.type) {
      case "replaying":
        return language.t("streaming.banner.replaying")
      case "resync_required":
      case "resync":
        return language.t("streaming.banner.resync")
      case "retry":
        return language.t("streaming.banner.retry")
      default:
        return ""
    }
  }
  ...
  return (
    <Show when={props.type}>
      <div class="flex items-center gap-2 px-3 py-2 rounded-md bg-surface-raised-base border border-border-weak-base text-text-base">
        ...
        <span class="text-12-regular">{bannerMessage()}</span>
      </div>
    </Show>
  )
}
