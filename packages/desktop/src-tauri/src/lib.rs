...
async fn initialize(app: AppHandle) {
    ...
    let sqlite_done = needs_sqlite_migration.then(|| {
        tracing::info!(
            path = %openhei_db_path().expect("failed to get db path").display(),
                    ...
                    Some(
                        async move {
                            ...
                            if let Some(err) = err {
                                ...
                                return Err(format!(
                                    "Failed to spawn OpenHei Server ({err}). Logs:\n{}",
                                    get_logs()
                                ));
                            }
...
async fn setup_server_connection(app: AppHandle) -> ServerConnection {
    ...
    ServerConnection::CLI {
        url: local_url,
        username: Some("openhei".to_string()),
...
fn get_sidecar_port() -> u32 {
    option_env!("OPENHEI_PORT")
        .map(|s| s.to_string())
        .or_else(|| std::env::var("OPENHEI_PORT").ok())
...
fn sqlite_file_exists() -> bool {
    let Ok(path) = openhei_db_path() else {
        return true;
    };
...
fn openhei_db_path() -> Result<PathBuf, &'static str> {
    ...
    Ok(data_home.join("openhei").join("openhei.db"))
}
