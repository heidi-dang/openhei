...
const CLI_INSTALL_DIR: &str = ".openhei/bin";
const CLI_BINARY_NAME: &str = "openhei";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("openhei-install.sh");
...
pub fn spawn_command(
    ...
    let mut envs = vec![
        (
            "OPENHEI_EXPERIMENTAL_ICON_DISCOVERY".to_string(),
            "true".to_string(),
        ),
        (
            "OPENHEI_EXPERIMENTAL_FILEWATCHER".to_string(),
            "true".to_string(),
        ),
        ("OPENHEI_CLIENT".to_string(), "desktop".to_string()),
        (
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut env_prefix = vec![
                "OPENHEI_EXPERIMENTAL_ICON_DISCOVERY=true".to_string(),
                "OPENHEI_EXPERIMENTAL_FILEWATCHER=true".to_string(),
                "OPENHEI_CLIENT=desktop".to_string(),
                "XDG_STATE_HOME=\"$HOME/.local/state\"".to_string(),
            ];
            env_prefix.extend(
                envs.iter()
                    .filter(|(key, _)| key != "OPENHEI_EXPERIMENTAL_ICON_DISCOVERY")
                    .filter(|(key, _)| key != "OPENHEI_EXPERIMENTAL_FILEWATCHER")
                    .filter(|(key, _)| key != "OPENHEI_CLIENT")
...
pub fn serve(
    ...
    let envs = [
        ("OPENHEI_SERVER_USERNAME", "openhei".to_string()),
        ("OPENHEI_SERVER_PASSWORD", password.to_string()),
    ];
    ...
    .expect("Failed to spawn openhei");
