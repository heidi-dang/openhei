...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("openhei-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("openhei_lib=debug,openhei_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("openhei_lib=info,openhei_desktop=info,sidecar=info")
        }
    });
